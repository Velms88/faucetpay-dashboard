...
async function openRealAdmin() {
  ...
  function selectTab(id) {
    ...
    else if (id === 'raw') renderRealRawData(bodyEl, realFaucetRows);
  }
  selectTab('targets');
}
...
function buildRawTableHtml(snaps) {
  ...
  for (const sym of coinOrder) {
    head += '<th>' + escapeAttr(sym) + '</th><th>Баланс ' + escapeAttr(sym) + '</th>';
  }
  head += '</tr></thead>';
  ...
  for (const s of snaps) {
    ...
    let totalPaid = 0;
    for (const c of s.coins) {
      const p = Number(prices[c.symbol]) || 0;
    ...
    for (const sym of coinOrder) {
      const c = s.coins.find((x) => x.symbol === sym);
      if (!c) { row += '<td>—</td><td>—</td>'; continue; }
      const p = Number(prices[sym]) || 0;
      const usd = (Number(c.balance) || 0) * p;
      row += '<td>' + escapeAttr(c.balance) + '</td>'
        + '<td>' + (p > 0 ? fmtMoney(usd) : '—') + '</td>';
    }
    row += '</tr>';
    body += row;
  }
  body += '</tbody>';
  return '<table class="admin-table">' + head + body + '</table>';
}

function renderRealRawData(container, faucetRows) {
  container.innerHTML = '';
  const wrap = document.createElement('div');

  const note = document.createElement('div');
  note.className = 'lock-note';
  note.innerHTML = '🔒 Просмотр сырых почасовых снимков API FaucetPay: полные данные по ВСЕМ монетам (включая с нулевым балансом) + курсы криптовалют. Хранится в Turso <code>raw_hourly</code> (последние 8 дней).';
  wrap.appendChild(note);
