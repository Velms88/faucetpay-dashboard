...
function renderRealTargets(container, cfg, faucetRows, overrides) {
  ...
  table.innerHTML = '<thead><tr><th>URL</th><th>Name</th><th>Cur</th><th>UII (0..' + (w3 * 100) + ')</th><th>Bonus (0..' + (w4 * 100) + ')</th><th>Payout type</th><th>Gateways</th><th>Age (мес.)</th><th>Enabled</th><th>Comment</th></tr></thead>';
  ...
  function renderBody() {
    ...
    filtered.forEach((f) => {
      ...
      tr.innerHTML =
        ...
        '<td><input class="admin-input num" data-f="uii" type="number" min="0" max="' + (w3 * 100) + '" step="0.1" value="' + uii + '" style="width:64px"/></td>' +
        '<td><input class="admin-input num" data-f="bonus_points" type="number" min="0" max="' + (w4 * 100) + '" step="0.1" value="' + bonus + '" style="width:64px"/></td>' +
      ...
      tr.querySelectorAll('.admin-input').forEach((inp) => {
        ...
        inp.addEventListener('input', () => {
          ...
          let v = inp.value;
          if (fld === 'uii' || fld === 'bonus_points' || fld === 'gateways_count' || fld === 'age_months') v = v === '' ? null : Number(v);
