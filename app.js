...
function renderRealRawData(container, faucetRows, rawUrls) {
  ...
  const note = document.createElement('div');
  note.className = 'lock-note';
  note.innerHTML = '🔒 Просмотр сырых почасовых снимков API FaucetPay. Столбцы: <code>Время (UTC)</code>, <code>paid_today (USD)</code> (сырое значение из ответа API, как есть), <code>total_users_paid</code> (сумма по монетам), далее для КАЖДОЙ монеты — столбец баланса и столбец <code>Курс &lt;монета&gt;</code> (в USD, на момент снимка, выделен зелёным), включая нулевые балансы. Таблица широкая — используйте горизонтальную прокрутку. Данные из Turso <code>raw_hourly</code> (последние 8 дней).';
  wrap.appendChild(note);
