...
function onFormulaChange() { realConfigDirty = true; }
...
function appendFormulaBlock(wrap, title, editor) {
  const block = document.createElement('div');
...
function renderRealFormulas(container) {
  ...
  tables.forEach((t) => {
    appendFormulaBlock(wrap, t.title, buildThresholdTable(rating[t.key] || (rating[t.key] = { thresholds: [] }), t.mode, t.fallback));
  });

  appendFormulaBlock(wrap, 'Блок 2 · Тип выплат (payout_type)', buildPayoutTypeEditor(rating.payout_type || (rating.payout_type = {})));
