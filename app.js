...
async function loadSandboxAdminData(forceDisk) {
  ...
  let lsHistory = null;
  if (!forceDisk) {
    lsConfig = loadFromLocalStorage(LS_CONFIG_KEY);
    lsHistory = loadFromLocalStorage(LS_HISTORY_KEY);
  }
  try {
    if (lsConfig && Array.isArray(lsConfig.targets)) {
      lsConfig.targets = lsConfig.targets.filter((t) => !DEMO_FAUCET_URLS.has(t && t.url));
    }
  } catch (e) { /* ignore */ }
  try {
    if (lsHistory && Array.isArray(lsHistory.faucets)) {
      lsHistory.faucets = lsHistory.faucets.filter((f) => !DEMO_FAUCET_URLS.has(f && f.url));
    }
  } catch (e) { /* ignore */ }
...
function saveRealConfig(toastMsg) {
  if (!realConfigCache) { showToast('Нет данных Real Mode для сохранения', 'error'); return; }
  const ok = saveToLocalStorage(LS_REAL_KEY, realConfigCache);
  if (ok) { if (toastMsg) showToast(toastMsg, 'success'); }
  else showToast('Не удалось сохранить Real Mode (localStorage недоступен)', 'error');
}
...
function syncMainTableFromSandbox() {
...
// and the real snapshots (history.json) — mirrors build_faucets.js enrichFaucets.
function computeRealRows(rawRows, hist, cfg) {
  const prices = Object.assign({}, cfg.crypto_prices_usd || {}, hist.crypto_prices_usd || {});
  const retention = (cfg.settings && cfg.settings.history_retention_days) || 7;
  const historyByUrl = new Map((hist.faucets || []).map((f) => [normUrl(f.url), f]));
  const targetMap = new Map((cfg.targets || []).map((t) => [normUrl(t.url), t]));
  const resultRows = rawRows.map((row) => {
    ...
    if (!f) {
      // Target exists but has no snapshot in history.json yet: render with
      // safe zero/default metrics instead of a stale raw row — never crash.
      try {
        const emptyHist = {
          coins: {},
        ...
        const merged = mergeModeration(emptyHist, target);
        const healthRes = clientCalc.calculateHealthScore(merged, prices);
        const ratingRes = clientCalc.calculateRating(merged, healthRes, cfg);
    ...
    try {
      ...
      const merged = mergeModeration(f, target);
      const healthRes = clientCalc.calculateHealthScore(merged, prices);
      const ratingRes = clientCalc.calculateRating(merged, healthRes, cfg);
      return Object.assign({}, row, {
        health_score: healthRes.health_score,
...
async function openRealAdmin() {
  let cfg;
  try { cfg = await loadRealConfig(); } catch (e) { showToast('Не удалось загрузить faucet_config.json: ' + e.message, 'error'); return; }
  ...
  function selectTab(id) {
    ...
    bodyEl.innerHTML = '';
    if (id === 'targets') renderRealTargets(bodyEl, cfg, realUrls, realUrlNames);
  }
  selectTab('targets');
}

function buildRealUrlOptions(currentUrl, realUrls, realUrlNames) {
  const set = new Set(realUrls);
  if (currentUrl) set.add(currentUrl);
  return Array.from(set).sort((a, b) => a.localeCompare(b)).map((u) => {
    const name = realUrlNames.get(u) || u;
    return '<option value="' + escapeAttr(u) + '">' + escapeAttr(name) + '</option>';
  }).join('');
}

function renderRealTargets(container, cfg, realUrls, realUrlNames) {
  const wrap = document.createElement('div');
  wrap.innerHTML = '<p class="modal-hint">Список реальных кранов и их индивидуальные характеристики (targets). Управление формулами, весами и коэффициентами — только в Песочнице (вкладка «Формулы»). Изменения автоматически сохраняются в конфиг Real Mode (<code>faucet_config.json</code> / localStorage).</p>';
  const table = document.createElement('table');
  table.className = 'admin-table';
  table.innerHTML = '<thead><tr><th>URL</th><th>Label</th><th>UII</th><th>Payout type</th><th>Gateways</th><th>Age (мес.)</th><th>Enabled</th><th>Comment</th><th></th></tr></thead>';
  const tb = document.createElement('tbody');
  (cfg.targets || []).forEach((t, idx) => {
    const tr = document.createElement('tr');
    tr.innerHTML =
      '<td><input class="admin-input" list="real-url-' + idx + '" data-f="url" value="' + escapeAttr(t.url || '') + '" style="width:240px"/></td>' +
      '<datalist id="real-url-' + idx + '">' + buildRealUrlOptions(t.url, realUrls, realUrlNames) + '</datalist>' +
      ...
      '<td><input class="admin-input" data-f="comment" value="' + escapeAttr(t.comment || '') + '" style="width:140px"/></td>' +
      '<td><button class="btn btn-danger" data-remove>✕</button></td>';
    tr.querySelector('[data-f="payout_type"]').value = t.payout_type || 'mixed';

    const urlInp = tr.querySelector('[data-f="url"]');
    urlInp.addEventListener('input', () => { t.url = urlInp.value; });
    urlInp.addEventListener('change', () => {
      t.url = urlInp.value.trim();
      if (realUrlNames.has(t.url)) {
        t.label = realUrlNames.get(t.url);
        const lbl = tr.querySelector('[data-f="label"]');
        if (lbl) lbl.value = t.label;
      }
      saveRealConfig();
    });
    ...
    tr.querySelector('[data-remove]').addEventListener('click', () => {
      ...
      saveRealConfig();
      renderRealTargets(container, cfg, realUrls, realUrlNames);
    });
    tb.appendChild(tr);
  });
  table.appendChild(tb);
  ...
  addBtn.addEventListener('click', () => {
    const used = new Set((cfg.targets || []).map((x) => normUrl(x.url)));
    const avail = realUrls.filter((u) => !used.has(normUrl(u)));
    const pick = avail[0] || 'https://';
    const name = realUrlNames.get(pick) || pick;
    cfg.targets.push({ url: pick, label: name, is_enabled: true, payout_type: 'mixed', gateways_count: 1, age_months: 0, uii: 1.0, comment: '' });
    saveRealConfig();
    renderRealTargets(container, cfg, realUrls, realUrlNames);
  });
...
function renderRealHistory(container, hist) {
  ...
  note.innerHTML = '🔒 Редактирование <code>history.json</code> и снимков API запрещено в Real Mode. Ниже — только просмотр текущего состояния.';
  wrap.appendChild(note);
  if (!hist) {
    const p = document.createElement('p');
    p.className = 'modal-hint';
    p.textContent = 'history.json недоступен для чтения (возможно, ещё не сгенерирован).';
    wrap.appendChild(p);
    container.appendChild(wrap);
    return;
  }
...
async function openSandboxAdmin() {
  ...
  footerEl.querySelector('#sb-reset').addEventListener('click', async () => {
    try {
      ...
      sandboxState.config.settings = JSON.parse(JSON.stringify(def.settings));
      sandboxState.config.targets = keepTargets;
      keepFaucets.forEach((f) => resetFaucetMetrics(f));
      sandboxState.history.faucets = keepFaucets;
      previewDetailUrl = null;
      selectTab(currentSandboxTabId);
      saveSandboxToLocal('Показатели и формула сброшены, краны сохранены');
    } catch (e) { showToast('Ошибка сброса: ' + e.message, 'error'); }
  });
  footerEl.querySelector('#sb-save').addEventListener('click', saveSandboxToLocal);
  footerEl.querySelector('#sb-export').addEventListener('click', exportSandboxState);
...
async function exportFormulasToReal() {
  ...
  try {
    const realCfg = await loadRealConfig();
    ...
    saveRealConfig('Конфигурация формул успешно экспортирована в Real Mode!');
    downloadJson('faucet_config.json', realCfg);
    recalculateRealModeWithNewConfig();
  } catch (e) {
    showToast('Ошибка экспорта в Real Mode: ' + e.message, 'error');
  }
}
...
function renderTargetsTab(container) {
  ...
  container.innerHTML = '';
  const wrap = document.createElement('div');
  wrap.innerHTML = '<p class="modal-hint">Список целей мониторинга (targets) — модерация каждого крана: URL, Label, UII, тип выплат, шлюзы, возраст, активность. Изменения применяются к Live Preview и к сохранённому <code>faucet_config.sandbox.json</code>. Без хардкода кранов.</p>';
  const table = document.createElement('table');
  table.className = 'admin-table';
...
function renderFormulasTab(container) {
  ...
  wrap.innerHTML = '<p class="modal-hint">Конструктор математической модели. Все параметры считываются из <code>faucet_config.sandbox.json</code> и мгновенно пересчитывают Live Preview. Без хардкода таблиц и монет.</p>';
...
function saveSandboxToLocal(toastMsg) {
  if (!sandboxState.config || !sandboxState.history) { showToast('Нет данных для сохранения', 'error'); return; }
  const okCfg = saveToLocalStorage(LS_CONFIG_KEY, sandboxState.config);
  const okHist = saveToLocalStorage(LS_HISTORY_KEY, sandboxState.history);
  if (okCfg && okHist) {
    showToast(toastMsg || 'Успешно сохранено', 'success');
    captureSandboxSnapshot();
    syncMainTableFromSandbox();
    updateSaveButton();
  } else {
    showToast('Не удалось сохранить в браузере (localStorage недоступен)', 'error');
  }
}

function resetFaucetMetrics(f) {
  if (!f) return;
  f.daily_health_medians_7d = [];
  f.payouts = {
  ...
  if (f.coins) {
    Object.keys(f.coins).forEach((c) => {
      if (!f.coins[c]) return;
      f.coins[c].daily_peaks = [];
      f.coins[c].recent_balances = [];
    });
  }
  f._tracking = { coins: {}, payouts: {}, hourly: [] };
}
...
function exportSandboxState() {
  if (!sandboxState.config || !sandboxState.history) { showToast('Нет данных для экспорта', 'error'); return; }
  downloadJson('faucet_config.sandbox.json', sandboxState.config);
  downloadJson('history.sandbox.json', sandboxState.history);
  showToast('Файлы выгружены (faucet_config.sandbox.json, history.sandbox.json)', 'success');
}
