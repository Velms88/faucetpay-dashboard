...
async function loadData() {
  ...
  try {
    ...
    // 3) Рекомпилируем рейтинг/здоровье (reference-данные всё ещё берём с Turso —
    //    это лёгкие агрегаты, тяжёлый список кранов уже в data.json).
    let finalRows = rows;
    try {
      if (cfg && hist) {
        const ov = await loadTargetOverrides('prod');
        ...
        try { medianHsByUrl = await loadLatestRawMedianHsByUrl(); } catch (e) { console.warn('median HS load failed:', e); }
        let payoutPauseByUrl = null;
        try { payoutPauseByUrl = await loadLatestRawPayoutPauseByUrl((cfg && cfg.settings && cfg.settings.history_retention_days) || 7); } catch (e) { console.warn('payout pause load failed:', e); }
        finalRows = computeRealRows(rows, hist, cfg, ov, peakMap, dailyVolByUrl, medianHsByUrl, payoutPauseByUrl);
      }
    } catch (e) {
