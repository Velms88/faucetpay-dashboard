...
const clientCalc = {
  ...
  calculateRating(faucetData, healthScoreResult, config) {
    ...
    // итераций (≈ часов) с последней выплаты, вычисленный из TOTAL_USERS_PAID таблицы
    // «Сырые данные» (loadLatestRawPayoutPauseByUrl). Динамический, как Health Score.
...
function buildRatingCalcSummaryHtml(b, ratingCfg) {
  ...
  html += '<div class="rc-block-head">Блок 1 · Платёжеспособность <span class="rc-weight">вес ' + pct(b.b1.weight) + '</span></div>';
  ...
  html += '<div class="rc-block">';
  html += '<div class="rc-block-head">Блок 2 · Надёжность <span class="rc-weight">вес ' + pct(b.b2.weight) + '</span></div>';
  html += '<div class="rc-sum-list">'
  ...
  html += '<div class="rc-block">';
  html += '<div class="rc-block-head">Блок 3 · UII <span class="rc-weight">plain additive (cap ' + b.b3.max + ')</span></div>';
  html += '<div class="rc-sum-list">'
  ...
  html += '<div class="rc-block">';
  html += '<div class="rc-block-head">Блок 4 · Bonus <span class="rc-weight">plain additive (cap ' + b.b4.max + ')</span></div>';
  html += '<div class="rc-sum-list">'
  ...
  if (b.hasFaucet) {
    const b1Weighted = (b && b.b1 && Number.isFinite(Number(b.b1.weighted))) ? Number(b.b1.weighted) : 0;
    const b2Weighted = (b && b.b2 && Number.isFinite(Number(b.b2.weighted))) ? Number(b.b2.weighted) : 0;
    const b3Score = (b && b.b3 && Number.isFinite(Number(b.b3.score))) ? Number(b.b3.score) : 0;
    const b4Score = (b && b.b4 && Number.isFinite(Number(b.b4.score))) ? Number(b.b4.score) : 0;
    const baseScore = b1Weighted + b2Weighted + b3Score + b4Score;
    const finalLimit = (b && Number.isFinite(Number(b.finalMax))) ? Number(b.finalMax) : 100;
    const finalScore = Math.min(finalLimit, baseScore);
...
function renderRatingCalcData(container, faucetRows, rawUrls) {
  ...
  async function loadAndRender() {
    ...
    try {
      ...
      const w1 = weights.block_1_solvency !== undefined ? weights.block_1_solvency : 0.58;
      const w2 = weights.block_2_reliability !== undefined ? weights.block_2_reliability : 0.20;
      ...
      const summaryRating = clientCalc.calculateRating(summaryFaucet, summaryHealth, rating);

      const breakdown = {
        b1: { dvPts: dvP, hsPts: hsP, paPts: paP, raw: b1Raw, score: b1Score, max: block1Max, weight: w1, weighted: b1Weighted },
        b2: {
          age: { val: ageMonths, pts: agePts },
          payoutType: { val: payoutTypeRaw, pts: ptPts },
          gateways: { val: gateways, pts: gwPts },
          raw: b2Raw, score: b2Score, max: block2Max, weight: w2, weighted: b2Weighted,
        },
        b3: { val: uii, score: b3Score, max: b3Max },
        b4: { val: bonus, score: b4Score, max: b4Max },
        hasFaucet: hasFaucet,
...
function buildBlockWeightsEditor() {
  ...
  function sync() {
    const s = (Number(weights.block_1_solvency) || 0) + (Number(weights.block_2_reliability) || 0) + (Number(weights.block_3_uii) || 0) + (Number(weights.block_4_bonus) || 0);
  ...
  block.appendChild(mk('block_3_uii', 'Блок 3 (UII)'));
  block.appendChild(mk('block_4_bonus', 'Блок 4 (Bonus Points)'));
