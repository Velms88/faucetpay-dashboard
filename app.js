...
const TURSO_READONLY_TOKEN = 'eyJhbGciOiJFZERTQSIsInR5cCI6IkpXVCJ9.eyJhIjoicm8iLCJpYXQiOjE3ODc4MjkyNDksImlkIjoiMDFhMDQyNjctYTcwMS03OTFkLWI5YTAtNTEwZTBiNjc0OTlhIiwia2lkIjoiUGlNWXZRQnNyeUNyY1NveUhvdUU0VUxCYjktSExyZXdERk1zRlJJd0NqZyIsInJpZCI6IjUxYTFmNTI1LTIxMmYtNDk3NS05ZmMwLWYzNWFlMTE3MzI3MiJ9.x9HInP-jpf1Mtl5nQ77_TTSXTIwfBBgP30aoCgGDbFjeDHAU3kM-NBqQL1Sh8vEQQfsFAy2ysUMkwpSWe_DRDA';
...
let page = 1;
let pageSize = 25;
let realSourceCache = null;
let dataLoadPromise = null;
let rawHourlyRowsPromise = null;
const rawSnapshotsCache = new Map();
const rawHistoryCache = new Map();

const APP_CACHE_DB = 'faucetpay-dashboard-cache';
const APP_CACHE_STORE = 'snapshots';
const APP_CACHE_KEY = 'real-v2';
...
function openAppCache() {
  return new Promise((resolve, reject) => {
    ...
    req.onupgradeneeded = () => req.result.createObjectStore(APP_CACHE_STORE);
...
async function readAppCache() {
  try {
    const db = await openAppCache();
    return await new Promise((resolve, reject) => {
      const req = db.transaction(APP_CACHE_STORE, 'readonly').objectStore(APP_CACHE_STORE).get(APP_CACHE_KEY);
      req.onsuccess = () => resolve(req.result || null);
      req.onerror = () => reject(req.error);
    });
  } catch (e) {
    console.warn('app cache read failed:', e.message);
    return null;
  }
}

async function writeAppCache(value) {
  try {
    const db = await openAppCache();
    await new Promise((resolve, reject) => {
      const req = db.transaction(APP_CACHE_STORE, 'readwrite').objectStore(APP_CACHE_STORE).put(value, APP_CACHE_KEY);
      req.onsuccess = resolve;
      req.onerror = () => reject(req.error);
    });
  } catch (e) {
    console.warn('app cache write failed:', e.message);
  }
}
...
async function clearAppCache() {
  try {
    const db = await openAppCache();
    await new Promise((resolve, reject) => {
      const req = db.transaction(APP_CACHE_STORE, 'readwrite').objectStore(APP_CACHE_STORE).delete(APP_CACHE_KEY);
      req.onsuccess = resolve;
      req.onerror = () => reject(req.error);
    });
  } catch (e) {
    console.warn('app cache clear failed:', e.message);
  }
}
...
const COLUMNS = [
  ...
  { key: 'block_3_uii', title: 'Блок 3: UII', num: true },
  { key: 'block_4_bonus', title: 'Блок 4: Bonus', num: true },
  ...
  COLUMNS.forEach((col) => {
    ...
    switch (col.key) {
      ...
      case 'block_2_reliability':
        ...
        break;
      case 'block_3_uii':
        td.textContent = (r.block_3_uii == null) ? '—' : fmtPct(r.block_3_uii);
        break;
      case 'block_4_bonus':
        td.textContent = (r.block_4_bonus == null) ? '—' : fmtPct(r.block_4_bonus);
        break;
...
async function loadData(forceRefresh) {
  ...
  dataLoadPromise = loadDataInternal(forceRefresh);
  try { return await dataLoadPromise; } finally { dataLoadPromise = null; }
}

async function loadDataInternal(forceRefresh) {
  hideError();
  ...
  try {
    if (forceRefresh) {
      await clearAppCache();
      realSourceCache = null;
      rawHourlyRowsPromise = null;
      rawSnapshotsCache.clear();
      rawHistoryCache.clear();
    }
    if (!forceRefresh) {
      const cached = await readAppCache();
      if (cached && Array.isArray(cached.finalRows) && cached.finalRows.length) {
        realSourceCache = cached.source || null;
        let cachedRows = cached.finalRows;
        if (cached.source && cached.source.faucetsJson && cached.source.hist) {
          try {
            const freshCfg = await loadRealConfig();
            const cfg = freshCfg || cached.source.cfg || {};
            const ov = await loadTargetOverrides('prod');
            const derived = cached.derived || {};
            const mapFromEntries = (entries) => new Map(Array.isArray(entries) ? entries : []);
            cachedRows = computeRealRows(
              normalizeResponse(cached.source.faucetsJson).rows,
              cached.source.hist,
              cfg,
              ov,
              mapFromEntries(derived.peakByUrl),
              mapFromEntries(derived.dailyVolByUrl),
              mapFromEntries(derived.medianHsByUrl),
              mapFromEntries(derived.payoutPauseByUrl)
            );
            realConfigCache = cfg;
          } catch (e) { console.warn('cached settings refresh failed:', e.message); }
        }
        allRows = cachedRows;
        recordCountEl.textContent = allRows.length;
        lastUpdatedEl.textContent = formatTimestamp(cached.lastSnapshotAt || cached.fetchedAt);
        render();
        return true;
      }
    }

    const source = await loadRealDataFromTurso();
    realSourceCache = source;
    const { faucetsJson, hist, cfg, lastSnapshotAt } = source;
    const derived = {};
    ...
    try {
      if (cfg && hist) {
        ...
        derived.peakByUrl = peakMap ? Array.from(peakMap.entries()) : [];
        let dailyVolByUrl = null;
        try { dailyVolByUrl = await loadLatestRawDailyVolumeByUrl(); } catch (e) { console.warn('daily volume load failed:', e); }
        derived.dailyVolByUrl = dailyVolByUrl ? Array.from(dailyVolByUrl.entries()) : [];
        let medianHsByUrl = null;
        try { medianHsByUrl = await loadLatestRawMedianHsByUrl(); } catch (e) { console.warn('median HS load failed:', e); }
        derived.medianHsByUrl = medianHsByUrl ? Array.from(medianHsByUrl.entries()) : [];
        let payoutPauseByUrl = null;
        try { payoutPauseByUrl = await loadLatestRawPayoutPauseByUrl((cfg.settings && cfg.settings.history_retention_days) || 7); } catch (e) { console.warn('payout pause load failed:', e); }
        derived.payoutPauseByUrl = payoutPauseByUrl ? Array.from(payoutPauseByUrl.entries()) : [];
        finalRows = computeRealRows(rows, hist, cfg, ov, peakMap, dailyVolByUrl, medianHsByUrl, payoutPauseByUrl);
      }
    } catch (e) {
...
refreshBtn.addEventListener('click', async () => {
  ...
  refreshBtn.textContent = '⟳ Полное обновление…';
  try {
    const ok = await loadData(true);
    refreshBtn.textContent = ok ? '✓ Обновлено' : '⚠ Ошибка обновления';
    setTimeout(() => { refreshBtn.textContent = originalText; }, 2500);
  } finally {
    refreshBtn.disabled = false;
    if (refreshBtn.textContent === '⟳ Полное обновление…') refreshBtn.textContent = originalText;
  }
});
...
const clientCalc = {
  ...
  calculateRating(faucetData, healthScoreResult, config) {
    ...
    const zeroResult = () => ({
      ...
      block_1: { score: 0.0, weighted: 0.0, enabled: block1Enabled, details: { daily_volume_pts: 0, health_pts: 0, activity_pts: 0 } },
      block_2: { score: 0.0, weighted: 0.0, enabled: block2Enabled, details: { age_pts: 0, payout_type_pts: 0, gateways_pts: 0 } },
      block_3: { score: 0.0, weighted: 0.0, enabled: block3Enabled },
      block_4: { score: 0.0, weighted: 0.0, enabled: block4Enabled },
    });
    ...
    const healthRef = (payouts.health_reference != null)
      ? payouts.health_reference
    ...
    return {
      ...
      block_1: {
        ...
        details: { daily_volume_pts: block1Enabled ? dailyVolumePts : 0, health_pts: block1Enabled ? healthPts : 0, activity_pts: block1Enabled ? activityPts : 0 },
      },
      block_2: {
        ...
        details: { age_pts: block2Enabled ? agePts : 0, payout_type_pts: block2Enabled ? payoutTypePts : 0, gateways_pts: block2Enabled ? gatewaysPts : 0 },
      },
      block_3: {
...
function normDomain(u) {
  ...
  try {
    ...
    return raw.replace(/^[a-z][a-z\d+.-]*:\/\//i, '').split(/[/?#]/)[0]
...
function mergeModeration(historyFaucet, target) {
  ...
  if (target) {
    ['is_enabled', 'age_months', 'payout_type', 'gateways_count', 'uii', 'bonus_points', 'label'].forEach((k) => {
...
async function queryTursoBatch(statements) {
  const token = (typeof window !== 'undefined' && window.__TURSO_TOKEN) || TURSO_READONLY_TOKEN;
  const requests = statements.map((item) => ({
    type: 'execute',
    stmt: { sql: item.sql, args: (item.args || []).map(normArg) },
  }));
  ...
  return statements.map((item, index) => {
    const exec = data.results && data.results[index];
    if (!exec || exec.type !== 'ok' || !exec.response || !exec.response.result) return [];
    const result = exec.response.result;
    const cols = (result.cols || []).map((c) => c.name);
...
async function loadAllRawHourlyRows() {
  if (!rawHourlyRowsPromise) {
    rawHourlyRowsPromise = queryTurso(
      'SELECT url, snapshot_at, day, health_score, coins_json, prices_json FROM raw_hourly ORDER BY url, snapshot_at ASC'
    ).then((rows) => (rows || []).map((row) => {
      let coins = [];
      let prices = {};
      try { coins = JSON.parse(row.coins_json || '[]'); } catch (e) { console.warn('invalid raw coins_json:', row.url, row.snapshot_at); }
      try { prices = JSON.parse(row.prices_json || '{}'); } catch (e) { console.warn('invalid raw prices_json:', row.url, row.snapshot_at); }
      return Object.assign({}, row, { coins, prices });
    }));
  }
  return rawHourlyRowsPromise;
}
...
async function loadLatestRawHealthByUrl() {
  const out = new Map();
  const rows = await loadAllRawHourlyRows();
  const byUrl = new Map();
  for (const r of (rows || [])) {
  ...
  for (const [u, snaps] of byUrl) {
    const latest = snaps[snaps.length - 1]; // ASC order -> last row is the latest snapshot
...
// Блок 1, подблок «Суточный объём выплат» (daily_volume): эталон считается
// ИСКЛЮЧИТЕЛЬНО из таблицы «Сырые данные» (raw_hourly). Для каждых суток берём
// PAID_TODAY (USD) из ПОСЛЕДНЕГО снимка этих суток (это пиковая сумма выплат за
// день), затем берём МЕДИАНУ ряда этих посуточных значений по ВСЕЙ имеющейся
// истории (включая текущие сутки). Так же, как Health Score, пересчитывается на
// каждом новом снимке.
function computeRawDailyVolumeReference(snaps) {
  // Блок 1 (daily_volume): берём МАКСИМАЛЬНОЕ значение paid_today за КАЖДЫЕ
  // ЗАВЕРШЁННЫЕ сутки, затем среднее арифметическое по всем завершённым суткам.
  // Текущие (незавершённые) сутки в расчёт НЕ входят.
  const byDay = new Map();
  ...
  // последние по времени сутки считаем текущими (незавершёнными) и исключаем
  ...
  if (!peaks.length) return Math.max.apply(null, byDay.get(curDay)); // только текущие сутки
  const sum = peaks.reduce((a, b) => a + b, 0);
  return sum / peaks.length;
}

async function loadLatestRawDailyVolumeByUrl() {
  const out = new Map();
  const rows = await loadAllRawHourlyRows();
  const byUrl = new Map();
  for (const r of (rows || [])) {
    const u = normUrl(r.url);
    if (!byUrl.has(u)) byUrl.set(u, []);
    byUrl.get(u).push({ snapshot_at: r.snapshot_at, coins: r.coins });
  }
  for (const [u, snaps] of byUrl) out.set(u, computeRawDailyVolumeReference(snaps));
  return out;
}

async function loadLatestRawMedianHsByUrl() {
  // Блок 1, подблок «Баллы за здоровье» (health_score): берём «Медиану HS»
  // ПОСЛЕДНЕГО (текущего) снимка из таблицы «Сырые данные» (raw_hourly) и
  // сравниваем со шкалой. Пересчитывается на каждом новом снимке, как Health Score.
  const out = new Map();
  const rows = await loadAllRawHourlyRows();
  const byUrl = new Map();
  for (const r of (rows || [])) {
    const u = normUrl(r.url);
    if (!byUrl.has(u)) byUrl.set(u, []);
    byUrl.get(u).push({ snapshot_at: r.snapshot_at, day: r.day, coins: r.coins, prices: r.prices });
  }
  for (const [u, history] of byUrl) {
    if (!history.length) continue;
    const series = computeRawHealthSeries(history);
    // «Медиана HS» последнего (текущего) снимка = то же значение, что в колонке
    // таблицы «Сырые данные»: snaps — ТОЛЬКО снимки текущего (последнего) дня,
    // history — вся история (эталон по предыдущим дням).
...
// Блок 1, подблок «Пауза в выплатах» (payout_activity_hours). Эталон — количество
// итераций (≈ часов) с момента последней выплаты, вычисленное исключительно из
// колонки TOTAL_USERS_PAID таблицы «Сырые данные» (raw_hourly). Снимки делаются
// примерно раз в час, поэтому сравниваем TOTAL_USERS_PAID текущего снимка с
// предыдущими ПООЧЕРЁДНО (идя вглубь истории), пока не встретится снимок, у
// которого значение СТРОГО МЕНЬШЕ текущего — количество итераций и есть эталон.
// Статистика TOTAL_USERS_PAID обнуляется каждые сутки, поэтому:
//   1) текущий TOTAL_USERS_PAID == 0  -> эталон = потолок (retention*24+1) -> 0 баллов;
//   2) внутри текущего дня уменьшения нет (все снимки равны) -> циклы суммируются
//      по дням (каждый день смотрим отдельно, refVal = последний снимок дня), пока
//      не найдётся уменьшение внутри какого-то дня; сумма итераций = эталон;
//   3) комбинированный: дни со значением 0 трактуются как случай 1 внутри своего
//      цикла (refVal=0 -> уменьшения нет, все итерации дня суммируются), затем
//      переходим к предыдущему дню.
// Пересчитывается на каждом новом снимке (динамически, как Health Score).
// snaps: массив { snapshot_at, day, total_users_paid } для ОДНОГО крана,
//        отсортированный по snapshot_at ASC (старейший первым).
function computePayoutPause(snaps, retentionDays) {
  ...
  // Случай 1: текущий показатель равен 0 -> максимальная пауза -> 0 баллов.
  ...
  const dayKeys = Array.from(byDay.keys()).sort(); // по возрастанию
  let totalIters = 0;
  // Идём по дням от текущего (последнего) вглубь.
  for (let di = dayKeys.length - 1; di >= 0; di--) {
    const daySnaps = byDay.get(dayKeys[di]); // ASC
    const refVal = Number(daySnaps[daySnaps.length - 1].total_users_paid) || 0; // последний снимок дня
    let found = false;
  ...
  // Прошли все дни без уменьшения -> эталон ограничен потолком.
  return Math.min(totalIters, MAX);
}

async function loadLatestRawPayoutPauseByUrl(retentionDays) {
  // Блок 1, подблок «Пауза в выплатах»: эталон (кол-во итераций с последней
  // выплаты) берём из TOTAL_USERS_PAID таблицы «Сырые данные» (raw_hourly).
  if (!window.TURSO_CONFIG || !window.TURSO_CONFIG.url) return null;
  const out = new Map();
  try {
    const rows = await loadAllRawHourlyRows();
    const byUrl = new Map();
    for (const r of (rows || [])) {
      ...
      let paid = 0;
      try {
        if (r.coins.length) paid = Number(r.coins[0].total_users_paid) || 0;
      } catch (_) {}
...
async function loadRealDataFromTurso() {
  let batchRows;
  try {
    batchRows = await queryTursoBatch([
      { sql: "SELECT data FROM configs WHERE mode='prod'" },
      { sql: "SELECT raw_json, health_score, rating, rating_grade FROM faucets WHERE mode='prod'" },
      { sql: 'SELECT url, data FROM history_faucets' },
      { sql: 'SELECT MAX(snapshot_at) AS mx FROM raw_hourly' },
    ]);
  } catch (e) {
    console.warn('Turso batch read failed, falling back to separate reads:', e.message);
    batchRows = await Promise.all([
      readTurso("SELECT data FROM configs WHERE mode='prod'"),
      readTurso("SELECT raw_json, health_score, rating, rating_grade FROM faucets WHERE mode='prod'"),
      queryTurso('SELECT url, data FROM history_faucets'),
      queryTurso('SELECT MAX(snapshot_at) AS mx FROM raw_hourly'),
    ]);
  }
  const [cfgRows, faucetRows, histRows, snapshotRows] = batchRows;
  ...
  const lastSnapshotAt = snapshotRows && snapshotRows.length && snapshotRows[0].mx
    ? snapshotRows[0].mx
    : null;
...
async function loadRawFullHistory(url) {
  ...
  if (rawHistoryCache.has(key)) return rawHistoryCache.get(key);
  ...
  rawHistoryCache.set(key, out);
  return out;
}

// Load full raw hourly snapshots for a faucet + day from Turso raw_hourly.
async function loadRawSnapshots(url, day) {
  const key = normUrl(url) + '|' + String(day || '');
  if (rawSnapshotsCache.has(key)) return rawSnapshotsCache.get(key);
  const rows = await queryTurso(
  ...
  rawSnapshotsCache.set(key, result);
  return result;
}

// Latest calendar day ACTUALLY present in raw_hourly. Used to anchor the date
// pickers in the "Сырые данные API" and "Rating Calc" admin tabs so they offer
// ONLY the days that really exist in the data (and stay frozen when no new
// snapshots are written), instead of sliding forward with the real calendar.
async function getLatestRawDay() {
  try {
    const rows = await queryTurso('SELECT MAX(day) AS d FROM raw_hourly');
    const d = rows && rows[0] && rows[0].d;
    if (d) return String(d).slice(0, 10);
  } catch (e) {
    console.warn('getLatestRawDay failed:', e && e.message ? e.message : e);
  }
...
async function saveTargetOverridesToTurso(mode, overridesMap, dirtySet) {
  ...
  const existingRows = await tursoAdminQuery(
    'SELECT url FROM faucet_target_overrides WHERE mode = ?',
    [mode]
  );
  ...
  (existingRows || []).forEach((row) => {
    ...
    if (!key) return;
    if (!existingUrlsByKey.has(key)) existingUrlsByKey.set(key, []);
    existingUrlsByKey.get(key).push(row.url);
  });
  ...
  keys.forEach((key) => {
    ...
    const data = Object.assign({}, ov, { url: storageUrl });
    (existingUrlsByKey.get(key) || []).forEach((existingUrl) => {
      requests.push({ type: 'execute', stmt: {
        sql: 'DELETE FROM faucet_target_overrides WHERE mode = ? AND url = ?',
        args: [V(mode), V(existingUrl)],
      } });
    });
    requests.push({ type: 'execute', stmt: {
      ...
      args: [V(mode), V(storageUrl), V(JSON.stringify(data)), V(now)],
    } });
  });
  requests.push({ type: 'close' });
  const res = await fetch(`${TURSO_DB_URL}/v2/pipeline`, {
  ...
  for (const key of keys) {
    ...
    const expected = Object.assign({}, ov, { url: key });
    if (!saved || JSON.stringify(JSON.parse(saved)) !== JSON.stringify(expected)) {
...
function defaultFormulaConfig() {
  return {
    ...
    health: {},
    rating_thresholds: {},
    rating: { block_weights: { block_1_solvency: 0.70, block_2_reliability: 0.30, block_3_uii: 0.20, block_4_bonus: 0.02, block_1_enabled: true, block_2_enabled: true, block_3_enabled: true, block_4_enabled: false } },
    targets: [],
  };
}
...
function computeRealRows(rawRows, hist, cfg, overridesMap, peakByUrl, dailyVolByUrl, medianHsByUrl, payoutPauseByUrl) {
  ...
  const resultRows = rawRows.map((row) => {
    ...
    try {
      ...
      // Блок 1 daily_volume: эталон берём живьём из raw_hourly (последний снимок
      // каждых суток -> PAID_TODAY USD -> медиана), а не из history_faucets.
      if (dailyVolByUrl && dailyVolByUrl.has(key)) {
        if (!merged.payouts) merged.payouts = {};
        merged.payouts.daily_volume_reference = dailyVolByUrl.get(key);
      }
      if (medianHsByUrl && medianHsByUrl.has(key)) {
        if (!merged.payouts) merged.payouts = {};
        merged.payouts.health_reference = medianHsByUrl.get(key);
      }
      if (payoutPauseByUrl && payoutPauseByUrl.has(key)) {
        if (!merged.payouts) merged.payouts = {};
        merged.payouts.payout_activity_hours = payoutPauseByUrl.get(key);
      }
      ...
      return Object.assign({}, row, {
        ...
        // Rating transparency: block scores.
        ...
        block_3_uii: ratingRes.block_3 ? ratingRes.block_3.score : null,
        block_4_bonus: ratingRes.block_4 ? ratingRes.block_4.score : null,
...
async function openRealAdmin() {
  ...
  const tabs = [
    ...
    { id: 'ratingcalc', label: 'Rating Calc' },
  ];
  tabs.forEach((t, i) => {
  ...
  function selectTab(id) {
    ...
    if (saveBtn) saveBtn.style.display = (id === 'raw' || id === 'ratingcalc') ? 'none' : '';
    ...
    else if (id === 'ratingcalc') renderRatingCalcData(bodyEl, realFaucetRows, realRawUrls);
  }
  bodyEl.innerHTML = '<p class="modal-hint">⏳ Загрузка данных из БД…</p>';
  ...
  let realFaucetRows = [];
  try {
    const source = realSourceCache || await loadRealDataFromTurso();
    realSourceCache = source;
    const { faucetsJson } = source;
...
function renderRealTargets(container, cfg, faucetRows, overrides) {
  const defaults = { is_enabled: true, payout_type: '', gateways_count: 0, age_months: 0, uii: 0, bonus_points: 0, comment: '' };
  const baseTarget = (url) => (cfg.targets || []).find((t) => normUrl(t.url) === normUrl(url)) || null;
  container.innerHTML = '';
  const rw = (cfg.rating && cfg.rating.block_weights) || {};
  const w3 = rw.block_3_uii != null ? rw.block_3_uii : 0.20;
  const w4 = rw.block_4_bonus != null ? rw.block_4_bonus : 0.02;
  ...
  table.innerHTML = '<thead><tr><th>Domain</th><th>Name</th><th>UII (0.' + (w3 * 100) + ')</th><th>Bonus (0.' + (w4 * 100) + ')</th><th>Payout type</th><th>Gateways</th><th>Age (мес.)</th><th>Enabled</th><th>Comment</th></tr></thead>';
  ...
  const captureEdit = (e) => {
    const inp = e.target;
    const tr = inp.closest('tr[data-url]');
    const fld = inp.dataset && inp.dataset.f;
    if (!tr || !fld) return;

    const key = tr.dataset.url;
    let t = overrides.get(key);
    if (!t) t = Object.assign({ url: tr.dataset.sourceUrl }, baseTarget(tr.dataset.sourceUrl) || defaults);
    else t = Object.assign({}, t);
    let v = inp.value;
    if (fld === 'uii' || fld === 'bonus_points' || fld === 'gateways_count' || fld === 'age_months') {
      if (v === '') v = null;
      else {
        let n = Number(v);
        if (isNaN(n)) n = 0;
        if (fld === 'uii' || fld === 'bonus_points') n = Math.trunc(n);
        v = n;
        if (fld === 'uii' || fld === 'bonus_points') inp.value = n;
      }
    }
  ...
  tb.addEventListener('input', captureEdit);
  tb.addEventListener('change', captureEdit);

  let term = '';
  function renderBody() {
    tb.innerHTML = '';
    const unique = new Map();
    (faucetRows || []).forEach((f) => {
      const domain = normDomain(f.url);
      if (domain && !unique.has(domain)) unique.set(domain, f);
    });
    const filtered = Array.from(unique.entries())
      .filter(([domain, f]) => !term || domain.indexOf(term) !== -1 || (f.name || '').toLowerCase().indexOf(term) !== -1)
      .sort(([domainA], [domainB]) => domainA.localeCompare(domainB, 'en', { sensitivity: 'base' }))
      .map(([, f]) => f);
    filtered.forEach((f) => {
      const key = normUrl(f.url);
      const domain = normDomain(f.url);
      const ov = overrides.get(key) || baseTarget(f.url) || defaults;
      const uii = def(ov.uii, 0);
      const bonus = def(ov.bonus_points, 0);
      ...
      const tr = document.createElement('tr');
      tr.dataset.url = key;
      tr.dataset.sourceUrl = f.url;
      tr.innerHTML =
        '<td title="' + escapeAttr(f.url || '') + '">' + escapeAttr(domain) + '</td>' +
        '<td>' + escapeAttr(f.name || '') + '</td>' +
        '<td><input class="admin-input num" data-f="uii" type="number" min="0" max="' + (w3 * 100) + '" step="1" value="' + uii + '" style="width:64px"/></td>' +
        '<td><input class="admin-input num" data-f="bonus_points" type="number" min="0" max="' + (w4 * 100) + '" step="1" value="' + bonus + '" style="width:64px"/></td>' +
...
// Build the "Rating Calc" table: for every snapshot of the selected day we show
// the three Блок-1 sub-block indicators (exactly the values the rating uses) and
// the points each one currently yields — so the rating math can be watched in
// dynamics, directly from the real raw_hourly records.
//   Время (UTC) | daily_volume_usd | Баллы | HS_Median | Баллы | payout_activity_hours | Баллы
function buildRatingCalcTableHtml(daySnaps, dvArr, hsArr, paArr, dvPts, hsPts, paPts) {
  let head = '<thead><tr>'
    + '<th>Время (UTC)</th>'
    + '<th>daily_volume_usd</th><th class="rc-pts rc-pts-dv">Баллы (объём)</th>'
    + '<th>HS_Median</th><th class="rc-pts rc-pts-hs">Баллы (HS)</th>'
    + '<th>payout_activity_hours</th><th class="rc-pts rc-pts-pa">Баллы (пауза)</th>'
    + '</tr></thead>';

  let body = '<tbody>';
  for (let i = 0; i < daySnaps.length; i++) {
    const t = escapeAttr(hourLabelFromIso(daySnaps[i].snapshot_at));
    const dv = (dvArr[i] != null) ? fmtUsd(dvArr[i]) : '—';
    const hs = (hsArr[i] != null) ? escapeAttr(String(hsArr[i])) : '—';
    const pa = (paArr[i] != null) ? escapeAttr(String(paArr[i])) : '—';
    body += '<tr' + (i === daySnaps.length - 1 ? ' class="rc-current"' : '') + '>'
      + '<td>' + t + '</td>'
      + '<td>' + dv + '</td>'
      + '<td class="rc-pts rc-pts-dv">' + (dvPts[i] != null ? escapeAttr(String(dvPts[i])) : '—') + '</td>'
      + '<td>' + hs + '</td>'
      + '<td class="rc-pts rc-pts-hs">' + (hsPts[i] != null ? escapeAttr(String(hsPts[i])) : '—') + '</td>'
      + '<td>' + pa + '</td>'
      + '<td class="rc-pts rc-pts-pa">' + (paPts[i] != null ? escapeAttr(String(paPts[i])) : '—') + '</td>'
      + '</tr>';
  }
  body += '</tbody>';
  return '<div class="raw-table-scroll"><table class="admin-table">' + head + body + '</table></div>';
...
// Сводка рейтинга для вкладки Rating Calc: Блоки 2–4 (статичные поля крана) и
// итоговый рейтинг. Полностью повторяет логику clientCalc.calculateRating, чтобы
// одна вкладка показывала весь расчёт целиком (Блок 1 — в динамике выше).
function buildRatingCalcSummaryHtml(b, ratingCfg) {
  function row(k, v) { return '<div class="rc-row"><span class="rc-k">' + k + '</span><span class="rc-v">' + v + '</span></div>'; }
  function subrow(k, v) { return '<div class="rc-row rc-sub"><span class="rc-k">' + k + '</span><span class="rc-v">' + (v == null ? '—' : v) + '</span></div>'; }
  function wrow(k, v) { return '<div class="rc-row rc-w"><span class="rc-k">' + k + '</span><span class="rc-v">' + (v == null ? '—' : round(v, 2)) + '</span></div>'; }
  const pct = (w) => (Number(w) * 100).toFixed(0) + '%';
  const esc = (s) => (s == null || s === '' ? '—' : escapeAttr(String(s)));
  const num = (n) => (n == null ? '—' : String(n));
  const safeGrade = (g) => (g == null || g === '' ? '—' : escapeAttr(String(g)));

  let html = '<div class="rc-summary"><h4 class="rc-summary-title">Сводка рейтинга · Блоки 1–4 (текущие значения крана)</h4>';

  const b1Style = b.b1.enabled === false ? ' style="opacity:0.4"' : '';
  const b1Tag = b.b1.enabled === false ? ' <span style="color:#e74c3c;font-weight:600">[ОТКЛЮЧЁН]</span>' : '';
  html += '<div class="rc-block"' + b1Style + '>';
  html += '<div class="rc-block-head">Блок 1 · Платёжеспособность <span class="rc-weight">вес ' + pct(b.b1.weight) + '</span>' + b1Tag + '</div>';
  html += '<div class="rc-sum-list">'
    + row('daily_volume_usd', (b.b1.dvPts != null ? b.b1.dvPts + ' баллов' : '—'))
    + row('health_score (HS_Median)', (b.b1.hsPts != null ? b.b1.hsPts + ' баллов' : '—'))
    + row('payout_activity_hours', (b.b1.paPts != null ? b.b1.paPts + ' баллов' : '—'))
    + subrow('Сырая сумма подблоков', b.b1.raw)
    + row('Балл блока (cap ' + b.b1.max + ')', b.b1.score)
    + wrow('Взвешенный (×' + b.b1.weight + ')', b.b1.weighted)
    + '</div></div>';

  const b2Style = b.b2.enabled === false ? ' style="opacity:0.4"' : '';
  const b2Tag = b.b2.enabled === false ? ' <span style="color:#e74c3c;font-weight:600">[ОТКЛЮЧЁН]</span>' : '';
  html += '<div class="rc-block"' + b2Style + '>';
  html += '<div class="rc-block-head">Блок 2 · Надёжность <span class="rc-weight">вес ' + pct(b.b2.weight) + '</span>' + b2Tag + '</div>';
  html += '<div class="rc-sum-list">'
    + row('age_months = ' + num(b.b2.age.val), (b.b2.age.pts != null ? b.b2.age.pts + ' баллов' : '—'))
    + row('payout_type = ' + esc(b.b2.payoutType.val), (b.b2.payoutType.pts != null ? b.b2.payoutType.pts + ' баллов' : '—'))
    + row('gateways_count = ' + num(b.b2.gateways.val), (b.b2.gateways.pts != null ? b.b2.gateways.pts + ' баллов' : '—'))
    + subrow('Сырая сумма подблоков', b.b2.raw == null ? '—' : b.b2.raw)
    + row('Балл блока (cap ' + b.b2.max + ')', b.b2.score == null ? '—' : b.b2.score)
    + wrow('Взвешенный (×' + b.b2.weight + ')', b.b2.weighted)
    + '</div></div>';

  const b3Style = b.b3.enabled === false ? ' style="opacity:0.4"' : '';
  const b3Tag = b.b3.enabled === false ? ' <span style="color:#e74c3c;font-weight:600">[ОТКЛЮЧЁН]</span>' : '';
  html += '<div class="rc-block"' + b3Style + '>';
  html += '<div class="rc-block-head">Блок 3 · UII <span class="rc-weight">plain additive (cap ' + b.b3.max + ')</span>' + b3Tag + '</div>';
  html += '<div class="rc-sum-list">'
    + row('uii = ' + (b.b3.val == null ? '—' : num(round(b.b3.val, 2))), (b.b3.score == null ? '—' : b.b3.score + ' баллов'))
    + wrow('Взнос в рейтинг (без веса)', b.b3.score)
    + '</div></div>';

  const b4Style = b.b4.enabled === false ? ' style="opacity:0.4"' : '';
  const b4Tag = b.b4.enabled === false ? ' <span style="color:#e74c3c;font-weight:600">[ОТКЛЮЧЁН]</span>' : '';
  html += '<div class="rc-block"' + b4Style + '>';
  html += '<div class="rc-block-head">Блок 4 · Bonus <span class="rc-weight">plain additive (cap ' + b.b4.max + ')</span>' + b4Tag + '</div>';
  html += '<div class="rc-sum-list">'
    + row('bonus_points = ' + (b.b4.val == null ? '—' : num(round(b.b4.val, 2))), (b.b4.score == null ? '—' : b.b4.score + ' баллов'))
    + wrow('Взнос в рейтинг (без веса)', b.b4.score)
    + '</div></div>';

  if (b.hasFaucet) {
    const baseScore = Number.isFinite(Number(b.baseRating)) ? Number(b.baseRating) : 0;
    const finalLimit = (b && Number.isFinite(Number(b.finalMax))) ? Number(b.finalMax) : 100;
    const finalScore = Number.isFinite(Number(b.finalRating)) ? Number(b.finalRating) : Math.min(finalLimit, baseScore);
    const finalGrade = (b && b.letterGrade != null && b.letterGrade !== '') ? b.letterGrade : 'F';
    html += '<div class="rc-total">'
      + '<div class="rc-total-row"><span>Базовый рейтинг (сумма взвешенных блоков)</span><span>' + round(baseScore, 2) + '</span></div>'
      + '<div class="rc-total-row rc-final"><span>Итоговый рейтинг (cap ' + finalLimit + ')</span><span>' + round(finalScore, 2) + ' · ' + safeGrade(finalGrade) + '</span></div>'
      + '</div>';
  } else {
    html += '<p class="modal-hint">Данные крана (Блоки 2–4) недоступны — кран отсутствует в таблице faucets.</p>';
  }
  html += '</div>';
  return html;
}
...
function renderRealRawData(container, faucetRows, rawUrls) {
  ...
  // Anchor the date window to the latest day ACTUALLY present in raw_hourly
  // (not the live calendar) so the picker stays frozen on the existing data
  // and only offers the days that really exist.
  getLatestRawDay().then((anchor) => {
    const aDt = new Date(anchor + 'T00:00:00Z');
    for (let i = 7; i >= 0; i--) {
      const v = new Date(aDt.getTime() - i * 86400000).toISOString().slice(0, 10);
      const o = document.createElement('option');
      o.value = v;
      o.textContent = v;
      dateSel.appendChild(o);
    }
    dateSel.value = anchor;
  });
...
// "Rating Calc" tab — mirrors the "Сырые данные API" tab UI (faucet combobox +
// day selector + scrollable table) but shows the three Блок-1 rating indicators
// and their points per snapshot, recomputed as of each snapshot from the real
// raw_hourly records (so the rating math is visible in dynamics).
function renderRatingCalcData(container, faucetRows, rawUrls) {
  container.innerHTML = '';
  const wrap = document.createElement('div');
  ...
  async function loadAndRender() {
    ...
    try {
      ...
      const retention = (cfg.settings && cfg.settings.history_retention_days) || 7;
      // Полная история снимков крана (все сутки) — эталоны Блока 1 считаются по
      // ВСЕЙ истории, поэтому для каждого снимка берём префикс истории ДО него.
      const rows = await queryTurso(
        'SELECT snapshot_at, day, coins_json, prices_json FROM raw_hourly WHERE url = ? ORDER BY snapshot_at ASC',
        [normUrl(url)]
      );
      const allHistory = (rows || []).map((r) => {
        const coins = JSON.parse(r.coins_json || '[]');
        let tup = 0;
        try { if (coins.length) tup = Number(coins[0].total_users_paid) || 0; } catch (_) {}
        return {
          snapshot_at: r.snapshot_at,
          day: r.day,
          coins: coins,
          prices: JSON.parse(r.prices_json || '{}'),
          total_users_paid: tup,
        };
      });
      if (!allHistory.length) {
        tableHost.innerHTML = '<p class="modal-hint">Нет снимков для выбранного крана.</p>';
        return;
      }
      const series = computeRawHealthSeries(allHistory);
      const daySnaps = allHistory
        .filter((s) => (s.day || s.snapshot_at.slice(0, 10)) === day)
        .slice()
        .sort((a, b) => (a.snapshot_at < b.snapshot_at ? -1 : a.snapshot_at > b.snapshot_at ? 1 : 0));
      if (!daySnaps.length) {
        tableHost.innerHTML = '<p class="modal-hint">Нет снимков за ' + escapeAttr(day) + ' для выбранного крана.</p>';
        return;
      }
      // HS_Median для каждого снимка = то же значение, что в колонке «Медиана HS»
      // таблицы «Сырые данные» (snaps — снимки текущего дня, history — вся история).
      const medianArr = computePerSnapshotMedianHs(daySnaps, allHistory, series);

      const dvArr = [], hsArr = medianArr, paArr = [];
      const dvPts = [], hsPts = [], paPts = [];
      // Block 1 эталон считаем по ВСЕЙ истории (как главная таблица: loadLatestRawDailyVolumeByUrl),
      // а не по prefix'у снимков выбранного дня — иначе текущий день исключается и dv=0 для кранов с одной датой.
      const dvRef = computeRawDailyVolumeReference(allHistory);
      const paRef = computePayoutPause(allHistory, retention);
      const dvPtsRef = getPointsFromThresholds(dvRef, rating.daily_volume_usd);
      for (let i = 0; i < daySnaps.length; i++) {
        const s = daySnaps[i];
        // Префикс истории ДО и ВКЛЮЧАЯ текущий снимок — для HS_Median.
        const prefix = allHistory.filter((h) => h.snapshot_at <= s.snapshot_at);
        const pa = computePayoutPause(prefix, retention);
        paArr.push(pa);
        dvArr.push(dvRef);
        hsArr[i] = hsArr[i];
        dvPts.push(dvPtsRef);
        hsPts.push(getPointsFromThresholds(hsArr[i], rating.health_score));
        paPts.push(getPointsFromThresholds(pa, rating.payout_activity_hours));
      }
      // --- Сводка рейтинга: Блоки 2–4 (поля крана) + итог, повторяет clientCalc.calculateRating ---
      const weights = rating.block_weights || {};
      const w1 = weights.block_1_solvency !== undefined ? weights.block_1_solvency : 0.70;
      const w2 = weights.block_2_reliability !== undefined ? weights.block_2_reliability : 0.30;
      const w3 = weights.block_3_uii !== undefined ? weights.block_3_uii : 0.20;
      const w4 = weights.block_4_bonus !== undefined ? weights.block_4_bonus : 0.02;
      const block1Max = rating.block_1_max_points !== undefined ? rating.block_1_max_points : 100.0;
      const block2Max = rating.block_2_max_points !== undefined ? rating.block_2_max_points : 100.0;
      const finalMax = rating.final_rating_max !== undefined ? rating.final_rating_max : 100.0;
      const letterThresholds = cfg.rating_thresholds || {};

      const last = daySnaps.length - 1;
      const dvP = dvPts[last], hsP = hsPts[last], paP = paPts[last];
      const b1Raw = dvP + hsP + paP;
      const b1Score = Math.min(block1Max, b1Raw);
      const b1Weighted = b1Score * w1;

      // --- Block 2: берём поля из history_faucets + faucet_target_overrides
      // (тот же алгоритм, что и в main table: computeRealRows + mergeModeration).
      // faucets.raw_json этих полей НЕ содержит — откуда тут были только нули.
      const overrideMap = await loadTargetOverrides('prod');
      const histRows = await queryTurso('SELECT url, data FROM history_faucets');
      const histMap = new Map((histRows || []).map((r) => [normUrl(r.url), JSON.parse(r.data)]));

      function getBlock2Fields(urlNorm) {
        const hist = histMap.get(urlNorm) || {
          coins: {}, payouts: { hours_since_last_payout: null, v_3h_actual: 0, v_3h_expected_median: 0, n_3h_actual: 0, n_3h_expected_median: 0, daily_volume_usd_7d_medians: [] },
          daily_health_medians_7d: [],
        };
        const override = overrideMap.get(urlNorm) || {};
        const merged = mergeModeration(hist, override);
        return {
          age_months: merged.age_months,
          payout_type: merged.payout_type,
          gateways_count: merged.gateways_count,
          uii: merged.uii,
          bonus_points: merged.bonus_points,
        };
      }

      const b2 = getBlock2Fields(normUrl(url));
      const ageMonths = b2.age_months;
      const payoutTypeRaw = b2.payout_type;
      const gateways = b2.gateways_count;
      const uii = b2.uii;
      const bonus = b2.bonus_points;
      const hasFaucet = true;

      const agePts = (ageMonths != null && !Number.isNaN(Number(ageMonths))) ? getPointsFromThresholds(ageMonths, rating.age_months) : 0;
      const payoutTypeMap = rating.payout_type || {};
      let ptPts = 0;
      if (payoutTypeRaw != null && payoutTypeRaw !== '') {
        ptPts = (payoutTypeMap[String(payoutTypeRaw).toLowerCase()] !== undefined)
          ? payoutTypeMap[String(payoutTypeRaw).toLowerCase()]
          : 0;
      }
      const gwPts = (gateways != null && !Number.isNaN(Number(gateways))) ? getPointsFromThresholds(gateways, rating.gateways_count) : 0;
      const b2Raw = agePts + ptPts + gwPts;
      const b2Score = Math.min(block2Max, b2Raw);
      const b2Weighted = b2Score * w2;

      const b3Max = w3 * 100, b4Max = w4 * 100;
      const b3Score = (uii != null && !Number.isNaN(Number(uii))) ? clamp(Number(uii), 0, b3Max) : 0;
      const b4Score = (bonus != null && !Number.isNaN(Number(bonus))) ? clamp(Number(bonus), 0, b4Max) : 0;

      // Важно: итог в сводке должен совпадать с main table. Поэтому считаем его
      // через тот же clientCalc.calculateRating, что и в общем рендере, а не по
      // отдельной ad-hoc формуле в UI.
      const summaryFaucet = {
        url: url,
        is_enabled: true,
        age_months: ageMonths,
        payout_type: payoutTypeRaw,
        gateways_count: gateways,
        uii: uii,
        bonus_points: bonus,
        daily_health_medians_7d: hsArr,
        coins: { _placeholder: { daily_peaks: [1], recent_balances: [1] } },
        payouts: {
          daily_volume_reference: dvRef,
          health_reference: hsArr[last],
          payout_activity_hours: paRef,
          hours_since_last_payout: paRef,
        },
      };
      const summaryHealth = clientCalc.calculateHealthScore(summaryFaucet, (cfg && cfg.crypto_prices_usd) || {});
      const summaryRating = clientCalc.calculateRating(summaryFaucet, summaryHealth, cfg);

      const breakdown = {
        b1: {
          dvPts: summaryRating.block_1.enabled ? dvP : null,
          hsPts: summaryRating.block_1.enabled ? hsP : null,
          paPts: summaryRating.block_1.enabled ? paP : null,
          raw: summaryRating.block_1.enabled ? b1Raw : 0,
          score: summaryRating.block_1.score,
          max: block1Max,
          weight: w1,
          weighted: summaryRating.block_1.weighted,
          enabled: summaryRating.block_1.enabled,
        },
        b2: {
          age: { val: ageMonths, pts: summaryRating.block_2.enabled ? agePts : 0 },
          payoutType: { val: payoutTypeRaw, pts: summaryRating.block_2.enabled ? ptPts : 0 },
          gateways: { val: gateways, pts: summaryRating.block_2.enabled ? gwPts : 0 },
          raw: summaryRating.block_2.enabled ? b2Raw : 0,
          score: summaryRating.block_2.score,
          max: block2Max,
          weight: w2,
          weighted: summaryRating.block_2.weighted,
          enabled: summaryRating.block_2.enabled,
        },
        b3: { val: uii, score: summaryRating.block_3.score, max: b3Max, enabled: summaryRating.block_3.enabled },
        b4: { val: bonus, score: summaryRating.block_4.score, max: b4Max, enabled: summaryRating.block_4.enabled },
        hasFaucet: hasFaucet,
        baseRating: summaryRating.base_rating,
        finalRating: summaryRating.final_rating,
        letterGrade: summaryRating.letter_grade,
        finalMax: finalMax,
      };

      tableHost.innerHTML = '<div class="rc-wrap">'
        + '<p class="modal-hint">Блок 1 — динамика подблоков по снимкам. Последняя строка (выделена) = текущее значение, используемое в рейтинге.</p>'
        + buildRatingCalcTableHtml(daySnaps, dvArr, hsArr, paArr, dvPts, hsPts, paPts)
        + buildRatingCalcSummaryHtml(breakdown, rating)
        + '</div>';
      // Жёстко выровнять ширину сводки под реальную ширину таблицы выше:
      // измеряем отрендеренную таблицу и ставим сводке ровно столько же пикселей.
      const rcTable = tableHost.querySelector('.rc-wrap .admin-table');
      const rcScroll = tableHost.querySelector('.rc-wrap .raw-table-scroll');
      const rcSummary = tableHost.querySelector('.rc-summary');
      if (rcTable && rcScroll && rcSummary) {
        const w = Math.min(rcTable.offsetWidth, rcScroll.clientWidth);
        rcSummary.style.width = w + 'px';
        rcSummary.style.boxSizing = 'border-box';
      }
    } catch (e) {
      const msg = (e && e.message) ? e.message : String(e);
      if (/no such table|unknown|raw_hourly/i.test(msg)) {
        tableHost.innerHTML = '<p class="modal-hint" style="color:var(--err,#c0392b)">Таблица <code>raw_hourly</code> ещё не создана или пуста. Дождитесь прогона pipeline (или запустите workflow вручную).</p>';
      } else {
        tableHost.innerHTML = '<p class="modal-hint" style="color:var(--err,#c0392b)">Ошибка загрузки: ' + escapeAttr(msg) + '</p>';
      }
    }
...
function buildBlockWeightsEditor() {
  ...
  const weights = rating.block_weights || (rating.block_weights = { block_1_solvency: 0.58, block_2_reliability: 0.20, block_3_uii: 0.20, block_4_bonus: 0.02 });
  if (weights.block_1_enabled === undefined) weights.block_1_enabled = true;
  if (weights.block_2_enabled === undefined) weights.block_2_enabled = true;
  if (weights.block_3_enabled === undefined) weights.block_3_enabled = true;
  if (weights.block_4_enabled === undefined) weights.block_4_enabled = false;
  ...
  function sync() {
    const enabledMask = [weights.block_1_enabled, weights.block_2_enabled, weights.block_3_enabled, weights.block_4_enabled];
    const rawW = [Number(weights.block_1_solvency) || 0, Number(weights.block_2_reliability) || 0, Number(weights.block_3_uii) || 0, Number(weights.block_4_bonus) || 0];
    const b1c = enabledMask[0] ? 100 * rawW[0] : 0;
    const b2c = enabledMask[1] ? 100 * rawW[1] : 0;
    const b3c = enabledMask[2] ? rawW[2] * 100 : 0;
    const b4c = enabledMask[3] ? rawW[3] * 100 : 0;
    const totalMax = b1c + b2c + b3c + b4c;
    const enabledCount = enabledMask.filter(Boolean).length;
    sumEl.textContent = 'Сумма весов: ' + (rawW[0] + rawW[1] + rawW[2] + rawW[3]).toFixed(2) +
      (enabledCount === 0 ? '  ⚠️ Все блоки отключены!' :
        '  · Вклад активных блоков: ' + totalMax.toFixed(1) + ' из 100');
    sumEl.style.color = enabledCount === 0 ? '#e74c3c' : (Math.abs(totalMax - 100) > 0.5 ? '#ffb020' : 'var(--text-dim)');
  }
  const blockDefs = [
    { key: 'block_1_solvency', enabledKey: 'block_1_enabled', label: 'Блок 1 (платёжеспособность)' },
    { key: 'block_2_reliability', enabledKey: 'block_2_enabled', label: 'Блок 2 (надёжность)' },
    { key: 'block_3_uii', enabledKey: 'block_3_enabled', label: 'Блок 3 (UII)' },
    { key: 'block_4_bonus', enabledKey: 'block_4_enabled', label: 'Блок 4 (Bonus Points)' },
  ];
  const rowEls = [];
  blockDefs.forEach((def) => {
    const cell = document.createElement('div');
    cell.className = 'admin-grid-cell bw-row';
    cell.style.display = 'flex';
    cell.style.alignItems = 'center';
    cell.style.gap = '10px';
    cell.style.opacity = weights[def.enabledKey] ? '1' : '0.45';
    cell.style.transition = 'opacity 0.2s';
    const toggleWrap = document.createElement('label');
    toggleWrap.className = 'toggle-switch';
    toggleWrap.style.flexShrink = '0';
    toggleWrap.title = weights[def.enabledKey] ? 'Блок участвует в расчёте рейтинга' : 'Блок отключён — не участвует в расчёте';
    const toggleInput = document.createElement('input');
    toggleInput.type = 'checkbox';
    toggleInput.checked = !!weights[def.enabledKey];
    ...
    labelEl.addEventListener('click', () => { toggleInput.click(); });
    const range = document.createElement('input');
    range.type = 'range'; range.min = '0'; range.max = '1'; range.step = '0.01';
    range.className = 'bw-range';
    range.value = weights[def.key] != null ? weights[def.key] : 0;
    range.style.width = '120px';
    range.disabled = !weights[def.enabledKey];
    const num = document.createElement('input');
    num.type = 'number'; num.min = '0'; num.max = '1'; num.step = '0.01';
    num.className = 'admin-input num bw-num';
    num.value = weights[def.key] != null ? weights[def.key] : 0;
    num.style.width = '70px';
    num.disabled = !weights[def.enabledKey];
    function setWeight(v) {
      weights[def.key] = Number(v);
      range.value = v; num.value = v;
      sync(); onFormulaChange();
    }
    function setEnabled(en) {
      weights[def.enabledKey] = en;
      range.disabled = !en;
      num.disabled = !en;
      cell.style.opacity = en ? '1' : '0.45';
      toggleWrap.title = en ? 'Блок участвует в расчёте рейтинга' : 'Блок отключён — не участвует в расчёте';
      sync(); onFormulaChange();
      refreshFormulaBadges();
    }
    toggleInput.addEventListener('change', () => setEnabled(toggleInput.checked));
    range.addEventListener('input', () => setWeight(range.value));
    num.addEventListener('input', () => setWeight(num.value));
    cell.appendChild(toggleWrap);
    cell.appendChild(labelEl);
    cell.appendChild(range);
    cell.appendChild(num);
    rowEls.push({ cell, def });
    block.appendChild(cell);
  });
  block.appendChild(sumEl);
  sync();
  block._refreshEnabled = function () {
    rowEls.forEach((r) => {
      const en = !!weights[r.def.enabledKey];
      r.cell.style.opacity = en ? '1' : '0.45';
      r.cell.querySelector('input[type="range"]').disabled = !en;
      r.cell.querySelector('input[type="number"]').disabled = !en;
    });
    sync();
  };
  return block;
...
function appendFormulaBlock(wrap, title, editor, getMax, getEnabled) {
  const block = document.createElement('div');
  ...
  if (getEnabled) {
    ...
    statusBadge.className = 'block-status-badge';
    const updateStatus = () => {
      const en = getEnabled();
      statusBadge.textContent = en ? 'активен' : 'отключён';
      statusBadge.style.color = en ? 'var(--text-dim)' : '#e74c3c';
      statusBadge.style.fontWeight = '600';
      statusBadge.style.fontSize = '12px';
      statusBadge.style.marginLeft = '8px';
      block.style.opacity = en ? '1' : '0.4';
      block.style.transition = 'opacity 0.2s';
    };
    updateStatus();
    formulaBadges.push(updateStatus);
...
function renderRealFormulas(container) {
  ...
  const weights = rating.block_weights || (rating.block_weights = {});
  if (weights.block_1_enabled === undefined) weights.block_1_enabled = true;
  if (weights.block_2_enabled === undefined) weights.block_2_enabled = true;
  if (weights.block_3_enabled === undefined) weights.block_3_enabled = true;
  if (weights.block_4_enabled === undefined) weights.block_4_enabled = false;
  ...
  const getBlock1Enabled = () => !!weights.block_1_enabled;
  const getBlock2Enabled = () => !!weights.block_2_enabled;
  ...
  tables.forEach((t) => {
    ...
    const getEn = t.block === 1 ? getBlock1Enabled : getBlock2Enabled;
    appendFormulaBlock(wrap, t.title, buildThresholdTable(rating[t.key] || (rating[t.key] = { thresholds: [] }), t.mode, t.fallback), () => sumMaxPoints(rating, keys), getEn);
  });

  appendFormulaBlock(wrap, 'Блок 2 · Тип выплат (payout_type)', buildPayoutTypeEditor(rating.payout_type || (rating.payout_type = {})), () => sumMaxPoints(rating, BLOCK2_KEYS), getBlock2Enabled);
