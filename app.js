...
function buildPricesEditor() {
  ...
  updBtn.addEventListener('click', async () => {
    if (!window.CryptoPrices || !window.CryptoPrices.fetchCryptoPrices) {
      showToast('Модуль CryptoPrices недоступен', 'error', updBtn);
      return;
    }
    ...
    try {
      ...
      if (N === 0) {
        showToast('API не вернул курсы ни для одной из ' + M + ' добавленных валют. Источники: ' + ok, 'error', updBtn);
      } else {
        showToast('Обновлено курсов: ' + N + ' из ' + M + ' добавленных. Источники: ' + ok, 'success', updBtn);
      }
    } catch (e) {
