...
// Полный путь загрузки из Turso (используется как фолбэк и по кнопке "Force Fresh DB").
async function loadFromTurso(forceFresh) {
  // При прямой загрузке из Turso (форс-рефреш или фолбэк) отключаем фоновый
  // опрос change_logs — он пересобирает таблицу из базового data.json + дельта,
  // что перебило бы только что полученные живые данные.
  stopDeltaPolling();
  lastStaticData = null;
...
// Гибридная инициализация: параллельно грузим статичный data.json (~20мс) и
// микро-дельту из change_logs (~50мс), патчим массив в памяти и сразу рисуем.
// При любом сбое (нет data.json / сеть) — фолбэк на полный Turso-путь.

// Базовый снимок последней успешной загрузки data.json — нужен, чтобы фоновый
// опрос change_logs мог каждый раз пересобрать список ИЗ ЧИСТОГО СНИМКА
// (дельта накапливается в БД, поэтому переприменяем её целиком, а не поверх
// уже применённой).
let lastStaticData = null;
let deltaPollTimer = null;

// Фоновый опрос change_logs: подхватывает правки админа (сделанные ПОСЛЕ
// сборки data.json) без перезагрузки страницы. Работает только на быстром
// пути (lastStaticData задан).
function startDeltaPolling() {
  if (deltaPollTimer) clearInterval(deltaPollTimer);
  deltaPollTimer = setInterval(pollDelta, 30000);
}
function stopDeltaPolling() {
  if (deltaPollTimer) { clearInterval(deltaPollTimer); deltaPollTimer = null; }
}
async function pollDelta() {
  if (!lastStaticData) return;
  try {
    const deltaLogs = await fetchChangeLogsFromTurso();
    if (!deltaLogs || !deltaLogs.length) return;
    const cfg = lastStaticData.config || null;
    const hist = lastStaticData.history || null;
    const ov = mapFromObj((lastStaticData.references || {}).targetOverrides);
    const base = (Array.isArray(lastStaticData.faucets) ? lastStaticData.faucets : []).map((r) => Object.assign({}, r));
    applyDeltaToSnapshot(base, cfg, deltaLogs, lastStaticData.generated_at, ov);
    const { fetchedAt, rows } = normalizeResponse({ fetched_at: lastStaticData.generated_at, data: { list_data: base } });
    if (!Array.isArray(rows) || !rows.length) return;
    const refs = lastStaticData.references || {};
    const peakMap = mapFromObj(refs.peakByUrl);
    const dailyVolByUrl = mapFromObj(refs.dailyVolByUrl);
    const medianHsByUrl = mapFromObj(refs.medianHsByUrl);
    const payoutPauseByUrl = mapFromObj(refs.payoutPauseByUrl);
    const finalRows = computeRealRows(rows, hist, cfg, ov, peakMap, dailyVolByUrl, medianHsByUrl, payoutPauseByUrl);
    allRows = finalRows;
    recordCountEl.textContent = finalRows.length;
    render();
  } catch (e) {
    console.warn('[delta poll] не удалось применить изменения:', e.message);
  }
}
...
async function loadData() {
  ...
  try {
    // 1) Параллельно: статичный snapshot + дельта из БД
    const [staticData, deltaLogs] = await Promise.all([
      fetch('./data.json?v=' + Date.now()).then((r) => {
        if (!r.ok) throw new Error('data.json HTTP ' + r.status);
        return r.json();
      }),
      fetchChangeLogsFromTurso(),
    ]);

    const cfg = staticData.config || null;
    if (cfg) realConfigCache = cfg;
    lastStaticData = staticData;
    const ov = mapFromObj((staticData.references || {}).targetOverrides);
    // Клонируем базовые записи, чтобы applyDelta мог мутировать их, не портя
    // оригинальный снимок (он нужен для фонового переприменения дельты).
    const listData = (Array.isArray(staticData.faucets) ? staticData.faucets : []).map((r) => Object.assign({}, r));
    const hist = staticData.history || null;
    const lastSnapshotAt = staticData.lastSnapshotAt || null;

    // 2) Патчим свежие правки админа ДО отрисовки (конфиг / удаление / оверрайды).
    applyDeltaToSnapshot(listData, cfg, deltaLogs, staticData.generated_at, ov);

    if (!listData.length) throw new Error('data.json содержит 0 кранов.');

    const { fetchedAt, rows } = normalizeResponse({ fetched_at: staticData.generated_at, data: { list_data: listData } });
    if (!Array.isArray(rows) || rows.length === 0) throw new Error('Не удалось извлечь записи из data.json.');

    // 3) Рекомпилируем рейтинг/здоровье. Все эталонные агрегаты (пик баланса, дневной
    //    объём, медиана HS, пауза выплат, таргет-оверрайды) уже зашиты в data.json.references
    //    — браузер НЕ обращается к Turso. Единственный живой запрос быстрого пути —
    //    крошечная дельта change_logs для свежих правок админа (выше).
    let finalRows = rows;
    try {
      if (cfg && hist) {
        const refs = staticData.references || {};
        const peakMap = mapFromObj(refs.peakByUrl);
        const dailyVolByUrl = mapFromObj(refs.dailyVolByUrl);
        const medianHsByUrl = mapFromObj(refs.medianHsByUrl);
        const payoutPauseByUrl = mapFromObj(refs.payoutPauseByUrl);
      ...
      console.warn('recompute skipped (using raw from data.json):', e);
    }
    ...
    console.warn('⚠️ Гибридная загрузка (data.json) не удалась, фолбэк на Turso:', err.message);
    stopDeltaPolling();
    try {
      await loadFromTurso(false);
    } catch (err2) {
      console.error(err2);
      allRows = [];
      recordCountEl.textContent = '0';
      lastUpdatedEl.textContent = '—';
      render();
      showError(err2.message || 'Не удалось загрузить данные.');
    }
  }
}

// Админ: принудительно прочитать полный дамп из Turso напрямую (минуя data.json).
async function forceFreshFromTurso() {
  try {
    await loadFromTurso(true);
  } catch (e) {
    showError(e.message || 'Не удалось загрузить данные из Turso.');
  }
}

document.getElementById('refresh-btn').addEventListener('click', loadData);

const forceFreshBtn = document.getElementById('force-fresh-btn');
if (forceFreshBtn) {
  forceFreshBtn.addEventListener('click', async () => {
    const ok = await requireAuth();
    if (ok) forceFreshFromTurso();
  });
}
...
// ===== Pre-render Delta Sync (статичный data.json + журнал change_logs) =====

// Лёгкий запрос микро-дельты из change_logs (только для точечных правок админа,
// сделанных ПОСЛЕ сборки текущего data.json). Read-only токен может SELECT.
async function fetchChangeLogsFromTurso() {
  try {
    return await queryTurso('SELECT faucet_id, field_name, new_value, created_at FROM change_logs');
  } catch (e) {
    console.warn('[delta] change_logs fetch failed:', e.message);
    return [];
  }
}

// Точечная запись правки админа в журнал change_logs (требуется Full-Access ключ).
async function logChangeToTurso(faucetId, fieldName, newValue) {
  const key = getTursoAdminKey();
  if (!key) return;
  try {
    // Самосоздание таблицы (на случай, если build_faucets ещё не отработал).
    await tursoAdminQuery(
      'CREATE TABLE IF NOT EXISTS change_logs (id INTEGER PRIMARY KEY AUTOINCREMENT, faucet_id TEXT NOT NULL, field_name TEXT NOT NULL, new_value TEXT NOT NULL, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)'
    );
    await tursoAdminWrite('INSERT INTO change_logs (faucet_id, field_name, new_value) VALUES (?, ?, ?)', [
      String(faucetId), String(fieldName), String(newValue),
    ]);
  } catch (e) {
    console.warn('[delta] change_logs insert failed:', e.message);
  }
}

// Ключ сопоставления записи крана из change_logs с объектом в списке (data.json).
function faucetMatchKey(rec) {
  if (rec && rec.id != null && String(rec.id).trim() !== '') return String(rec.id);
  const u = rec && rec.url ? normUrl(rec.url) : '';
  const c = rec && rec.currency ? String(rec.currency).toUpperCase() : '';
  return u + '|' + c;
}

// Патчим статичный snapshot прямо в памяти браузера ДО отрисовки:
//  - конфиг целиком (faucet_id='__config__', field_name='config')
//  - удаление крана (field_name='__delete__')
//  - точечное поле крана (faucet_id=id/url, field_name=имя поля, тип совпадает с текущим)
function applyDeltaToSnapshot(listData, cfg, deltaLogs, generatedAt, overridesMap) {
  if (!deltaLogs || !deltaLogs.length || !Array.isArray(listData)) return;
  const gen = new Date(generatedAt).getTime();
  if (isNaN(gen)) return;
  const fresh = deltaLogs.filter((l) => {
    const t = new Date(l.created_at).getTime();
    return !isNaN(t) && t > gen;
  });
  if (!fresh.length) return;
  const byKey = new Map();
  for (const f of listData) byKey.set(faucetMatchKey(f), f);
  for (const l of fresh) {
    const fid = l.faucet_id;
    const fn = l.field_name;
    if (fid === '__config__' && fn === 'config') {
      try { Object.assign(cfg, JSON.parse(l.new_value)); } catch (e) {}
      continue;
    }
    if (fn === '__delete__') {
      const idx = listData.findIndex((f) => faucetMatchKey(f) === fid);
      if (idx >= 0) listData.splice(idx, 1);
      continue;
    }
    if (fn === 'target_overrides') {
      if (overridesMap) {
        try { overridesMap.set(fid, JSON.parse(l.new_value)); } catch (e) {}
      }
      continue;
    }
    const target = byKey.get(fid);
    if (target) {
      const cur = target[fn];
      if (typeof cur === 'number') target[fn] = Number(l.new_value);
      else if (typeof cur === 'boolean') target[fn] = (l.new_value === 'true' || l.new_value === '1');
      else target[fn] = l.new_value;
    }
  }
}
...
async function loadLatestRawDailyVolumeByUrl() {
  const out = new Map();
  const rows = await queryTurso('SELECT url, snapshot_at, coins_json FROM raw_hourly ORDER BY url, snapshot_at ASC');
  const byUrl = new Map();
  for (const r of (rows || [])) {
    const u = normUrl(r.url);
    if (!byUrl.has(u)) byUrl.set(u, []);
    byUrl.get(u).push({ snapshot_at: r.snapshot_at, coins: JSON.parse(r.coins_json || '[]') });
  }
...
async function loadLatestRawMedianHsByUrl() {
  ...
  const out = new Map();
  const rows = await queryTurso('SELECT url, snapshot_at, day, coins_json, prices_json FROM raw_hourly ORDER BY url, snapshot_at ASC');
  const byUrl = new Map();
  for (const r of (rows || [])) {
    const u = normUrl(r.url);
    if (!byUrl.has(u)) byUrl.set(u, []);
...
async function saveConfigToTurso(mode, config) {
  ...
  // Перед записью гарантируем полную структуру конфига, чтобы в БД никогда не
  // попал «обрезанный» конфиг без settings (иначе сборка data.json падает).
  // Используем ГЛУБОКИЙ merge вложенных объектов (rating/rating_thresholds/...),
  // иначе частичный config (например, только block_weights) стирал бы пороговые
  // таблицы рейтинга в Turso, и рейтинг превращался бы в нули.
  const def = defaultFormulaConfig();
  const safe = Object.assign({}, def, config || {});
  safe.rating = Object.assign({}, def.rating, (config && config.rating) || {});
  safe.rating_thresholds = Object.assign({}, def.rating_thresholds, (config && config.rating_thresholds) || {});
  safe.targets = Array.isArray(config && config.targets) ? config.targets : def.targets;
  safe.settings = Object.assign({}, def.settings, (config && config.settings) || {});
  safe.crypto_prices_usd = Object.assign({}, def.crypto_prices_usd, (config && config.crypto_prices_usd) || {});
  safe.health = Object.assign({}, def.health, (config && config.health) || {});
  ...
  const res = await fetch(`${TURSO_DB_URL}/v2/pipeline`, {
    ...
    headers: { 'Content-Type': 'application/json', Authorization: 'Bearer ' + token },
    body: JSON.stringify({
      requests: [{ type: 'execute', stmt: { sql, args: [V(mode), V(JSON.stringify(safe)), V(new Date().toISOString())] } }, { type: 'close' }],
    }),
  });
  const data = await res.json();
  ...
  // Точечная правка конфига попадает в журнал дельты (для гибридной загрузки).
  await logChangeToTurso('__config__', 'config', JSON.stringify(config));
  return true;
}
...
async function deleteFaucetFromTurso(id, mode) {
  const r = await tursoAdminWrite('DELETE FROM faucets WHERE id = ? AND mode = ?', [id, mode]);
  await logChangeToTurso(id, '__delete__', '');
  return r;
}
...
async function saveTargetOverridesToTurso(mode, overridesMap, dirtySet) {
  ...
  // Логируем каждую изменённую оверрайд-цель в change_logs, чтобы быстрый путь
  // (data.json + дельта) подхватил правку фоном, не дожидаясь пересборки snapshot.
  for (const key of keys) {
    const ov = overridesMap.get(key);
    if (ov) await logChangeToTurso(normUrl(ov.url || key), 'target_overrides', JSON.stringify(ov));
  }
...
function defaultFormulaConfig() {
  // NOTE: this default MUST be COMPLETE. saveConfigToTurso() uses it as a
  // safety net (Object.assign), so any rating sub-object omitted here would be
  // permanently WIPED from Turso on the next admin save (that was the root
  // cause of "all ratings became zero" — the threshold tables got erased).
  return {
    settings: { history_retention_days: 7, history_file: 'history.json', data_file: 'faucets.json', group_by: 'category' },
    crypto_prices_usd: {},
    health: {},
...
function buildRatingCalcSummaryHtml(b, ratingCfg) {
  ...
  if (b.hasFaucet) {
    html += '<div class="rc-total">'
      + '<div class="rc-total-row"><span>Базовый рейтинг (сумма взвешенных блоков)</span><span>' + round(b.baseRating, 2) + '</span></div>'
      + '<div class="rc-total-row rc-final"><span>Итоговый рейтинг (cap ' + b.finalMax + ')</span><span>' + round(b.finalRating, 2) + ' · ' + escapeAttr(b.letterGrade) + '</span></div>'
      + '</div>';
  } else {
...
function renderRatingCalcData(container, faucetRows, rawUrls) {
  ...
  async function loadAndRender() {
    ...
    try {
      ...
      for (let i = 0; i < daySnaps.length; i++) {
        const s = daySnaps[i];
        // Префикс истории ДО и ВКЛЮЧАЯ текущий снимок — именно так считаются
        // эталоны рейтинга («как если бы этот снимок был последним»).
        const prefix = allHistory.filter((h) => h.snapshot_at <= s.snapshot_at);
        const dv = computeRawDailyVolumeReference(prefix);
        const pa = computePayoutPause(prefix, retention);
        dvArr.push(dv);
        paArr.push(pa);
        dvPts.push(getPointsFromThresholds(dv, rating.daily_volume_usd));
      ...
      const frec = (faucetRows || []).find((r) => normUrl(r.url) === normUrl(url));
      const hasFaucet = !!frec;
      const ageMonths = frec ? Number(frec.age_months) || 0 : null;
      const payoutTypeRaw = frec ? frec.payout_type : null;
      const gateways = frec ? Number(frec.gateways_count) || 0 : null;
      const uii = frec ? Number(frec.uii) || 0 : null;
      const bonus = frec ? Number(frec.bonus_points) || 0 : null;

      const agePts = (ageMonths != null) ? getPointsFromThresholds(ageMonths, rating.age_months) : null;
      const payoutTypeMap = rating.payout_type || {};
      let ptPts;
      if (payoutTypeRaw == null || payoutTypeRaw === '') ptPts = 0;
      else ptPts = (payoutTypeMap[String(payoutTypeRaw).toLowerCase()] !== undefined)
        ? payoutTypeMap[String(payoutTypeRaw).toLowerCase()]
        : (payoutTypeMap.manual !== undefined ? payoutTypeMap.manual : 10.0);
      const gwPts = (gateways != null) ? getPointsFromThresholds(gateways, rating.gateways_count) : null;
      const b2Raw = (agePts != null && ptPts != null && gwPts != null) ? (agePts + ptPts + gwPts) : null;
      const b2Score = (b2Raw != null) ? Math.min(block2Max, b2Raw) : null;
      const b2Weighted = (b2Score != null) ? b2Score * w2 : null;

      const b3Max = w3 * 100, b4Max = w4 * 100;
      const b3Score = (uii != null) ? clamp(uii, 0, b3Max) : null;
      const b4Score = (bonus != null) ? clamp(bonus, 0, b4Max) : null;

      let baseRating = null, finalRating = null, letterGrade = '';
      if (b2Score != null && b3Score != null && b4Score != null) {
        baseRating = b1Weighted + b2Weighted + b3Score + b4Score;
        finalRating = Math.min(finalMax, baseRating);
        letterGrade = ratingFromScore(finalRating, letterThresholds).grade;
      }
      ...
      const breakdown = {
        ...
        hasFaucet: hasFaucet,
        baseRating: baseRating, finalRating: finalRating, letterGrade: letterGrade, finalMax: finalMax,
      };
