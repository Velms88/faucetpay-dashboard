...
async function queryTurso(sql, args) {
  ...
  const stmt = args ? { sql, args: args.map(toTursoArg) } : { sql };
...
function buildRawTableHtml(snaps) {
  const coinOrder = [];
  ...
  let head = '<thead><tr><th>Время (UTC)</th><th>paid_today (USD)</th><th>total_users_paid</th>';
  ...
  for (const s of snaps) {
    ...
    // paid_today (USD): sum over coins of (paid_today amount in coin) * (rate).
    // total_users_paid: sum of total_users_paid across all coins of the faucet.
    let paidUsd = 0;
    let totalPaid = 0;
    for (const sym of coinOrder) {
      const c = coinMap[sym];
      ...
      if (c) {
        paidUsd += (Number(c.paid_today) || 0) * p;
        totalPaid += (Number(c.total_users_paid) || 0);
      }
    }

    let row = '<tr><td>' + escapeAttr(hourLabelFromIso(s.snapshot_at)) + '</td>'
      + '<td>' + fmtMoney(paidUsd) + '</td>'
      + '<td>' + fmtInt(totalPaid) + '</td>';
    for (const sym of coinOrder) {
      const c = coinMap[sym];
      const bal = c ? (Number(c.balance) || 0) : 0;
      const p = Number(prices[sym]) || 0;
      row += '<td>' + escapeAttr(String(bal)) + '</td>'
        + '<td>' + (p > 0 ? escapeAttr(fmtMoney(p)) : '—') + '</td>';
    }
    row += '</tr>';
    body += row;
  }
...
function renderRealRawData(container, faucetRows, rawUrls) {
  ...
  const note = document.createElement('div');
  note.className = 'lock-note';
  note.innerHTML = '🔒 Просмотр сырых почасовых снимков API FaucetPay. Столбцы: <code>Время (UTC)</code>, <code>paid_today (USD)</code> (сумма paid_today по всем монетам × курс), <code>total_users_paid</code> (сумма по монетам), далее для КАЖДОЙ монеты — столбец баланса и столбец <code>Курс &lt;монета&gt;</code> (в USD, на момент снимка), включая нулевые балансы. Таблица широкая — используйте горизонтальную прокрутку. Данные из Turso <code>raw_hourly</code> (последние 8 дней).';
  wrap.appendChild(note);
  ...
  urlInput.setAttribute('list', 'raw-url-list');
  urlInput.placeholder = 'Выберите или введите URL крана...';
  urlInput.style.flex = '1';
  urlInput.style.minWidth = '300px';

  // Build the full set of selectable faucet URLs: every URL that has raw
  // snapshots, plus every URL from the faucets table. De-duplicated.
  const urlSet = new Set();
  const allUrls = [];
  const pushUrl = (u) => { if (u && !urlSet.has(u)) { urlSet.add(u); allUrls.push(u); } };
  ...
  if (firstUrl) urlInput.value = firstUrl;
  ...
  async function loadAndRender() {
    ...
    tableHost.innerHTML = buildRawTableHtml(snaps);
  }

  urlInput.addEventListener('change', loadAndRender);
  dateSel.addEventListener('change', loadAndRender);
  if (urlInput.value) loadAndRender();
}
