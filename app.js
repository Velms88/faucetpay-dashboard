...
async function loadLatestRawHealthByUrl() {
  const out = new Map();
  const rows = await queryTurso('SELECT url, snapshot_at, health_score FROM raw_hourly ORDER BY url, snapshot_at DESC');
  for (const r of (rows || [])) {
    const u = normUrl(r.url);
    if (!out.has(u)) out.set(u, r.health_score != null ? Number(r.health_score) : null);
  }
  return out;
}
...
async function loadRealDataFromTurso() {
  ...
  try {
    ...
    for (const r of listData) {
      ...
      if (v != null) r.health_score = v;
    }
  } catch (e) { console.warn('latest raw health override failed:', e); }
...
function buildRawTableHtml(snaps, totalByAt, healthByAt, peakValue) {
  ...
  let head = '<thead><tr><th>Время (UTC)</th><th>paid_today (USD)</th><th>total_users_paid</th><th>TOTAL_BALANCE (USD)</th><th>Current health</th>';
...
function renderRealRawData(container, faucetRows, rawUrls) {
  ...
  const note = document.createElement('div');
  note.className = 'lock-note';
  note.innerHTML = '🔒 Просмотр сырых почасовых снимков API FaucetPay. Столбцы: <code>Время (UTC)</code>, <code>paid_today (USD)</code> (сырое значение из ответа API, как есть), <code>total_users_paid</code> (сумма по монетам), <code>TOTAL_BALANCE (USD)</code> (суммарный баланс по всем монетам в USD по курсам снимка), <code>Current health</code> (упрощённый Health Score по <code>raw_hourly</code>: текущий <code>TOTAL_BALANCE</code> / абсолютный пик баланса в USD за ВСЮ историю, включая текущую строку, × 100), далее для КАЖДОЙ монеты — столбец баланса и столбец <code>Курс &lt;монета&gt;</code> (в USD, на момент снимка, выделен зелёным), включая нулевые балансы. Таблица широкая — используйте горизонтальную прокрутку. Данные из Turso <code>raw_hourly</code>.';
  wrap.appendChild(note);
