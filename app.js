...
async function loadData() {
  ...
  try {
    ...
    if (cfg) realConfigCache = cfg;
    const listData = Array.isArray(staticData.faucets) ? staticData.faucets : [];
    const hist = staticData.history || null;
    const lastSnapshotAt = staticData.lastSnapshotAt || null;

    // 2) Патчим свежие правки админа ДО отрисовки
    if (deltaLogs && deltaLogs.length) {
      const gen = new Date(staticData.generated_at).getTime();
      const freshLogs = deltaLogs.filter((l) => new Date(l.created_at).getTime() > gen);
      if (freshLogs.length) {
        console.log('⚡ [Pre-render] применяем ' + freshLogs.length + ' свежих правок из БД до отрисовки...');
        applyDeltaToSnapshot(listData, cfg, deltaLogs, staticData.generated_at);
      }
    }
    ...
    try {
      if (cfg && hist) {
        ...
        const ov = mapFromObj(refs.targetOverrides);
...
function applyDeltaToSnapshot(listData, cfg, deltaLogs, generatedAt) {
