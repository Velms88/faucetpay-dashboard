...
const COLUMNS = [
  ...
  { key: 'calc_coins', title: 'Расчёт по монетам', ellipsis: true },
  ...
  COLUMNS.forEach((col) => {
    ...
    switch (col.key) {
      ...
      case 'block_2_reliability':
        ...
        break;
      case 'calc_coins':
        td.textContent = (Array.isArray(r.calc_coins) && r.calc_coins.length) ? r.calc_coins.join(', ') : '—';
        break;
      case 'uii':
...
// stores per-snapshot `balance` (no separate peak column), so we use the snapshot
// balance as the value for each hour; every prior snapshot contributes one sample.
...
function computeRawSnapshotHealth(prevSnaps, curSnap) {
  if (!prevSnaps || !prevSnaps.length) return 0;
  const curPrices = (curSnap && curSnap.prices) || {};
  const curCoins = (curSnap && curSnap.coins) || [];
  const coinSet = new Set();
  curCoins.forEach((c) => { if (c && c.symbol) coinSet.add(c.symbol); });
  Object.keys(curPrices).forEach((s) => coinSet.add(s));
  const participating = [];
  coinSet.forEach((sym) => {
    let ok = true;
    for (const ps of prevSnaps) {
      const cm = (ps.coins || []).find((c) => c.symbol === sym);
      const bal = cm ? Number(cm.balance) || 0 : 0;
      if (bal <= 0) { ok = false; break; }
    }
    if (ok) participating.push(sym);
  });
  if (!participating.length) return 0;
  const baseline = {};
  participating.forEach((sym) => {
    const vals = [];
    for (const ps of prevSnaps) {
      const cm = (ps.coins || []).find((c) => c.symbol === sym);
      if (cm) vals.push(Number(cm.balance) || 0);
    }
    baseline[sym] = median(vals);
  });
  const healthPct = {};
  participating.forEach((sym) => {
    const vals = [];
    ...
    healthPct[sym] = base > 0 ? Math.min(100, (mean / base) * 100) : 0;
  });
  let totalUsd = 0;
  const usd = {};
  participating.forEach((sym) => {
    const price = Number(curPrices[sym]) || 0;
    const v = (baseline[sym] || 0) * price;
    usd[sym] = v; totalUsd += v;
  });
  if (totalUsd <= 0) return 0;
  let score = 0;
  participating.forEach((sym) => {
    const w = usd[sym] / totalUsd;
    score += (healthPct[sym] || 0) * w;
  });
  return Math.round(score * 100) / 100;
}

// Load a multi-day window of raw_hourly snapshots for a faucet so per-snapshot
// Health Score can be computed from prior history (up to 7 days back).
async function loadRawWindow(url, day, daysBack) {
  const out = [];
  ...
  for (let i = daysBack; i >= 0; i--) {
    ...
    const rows = await queryTurso(
      'SELECT snapshot_at, coins_json, prices_json, health_score FROM raw_hourly WHERE url = ? AND day = ? ORDER BY snapshot_at ASC',
      [normUrl(url), d]
    );
    for (const r of (rows || [])) {
      out.push({
  ...
  out.sort((a, b) => (a.snapshot_at < b.snapshot_at ? -1 : a.snapshot_at > b.snapshot_at ? 1 : 0));
  return out;
}
...
function computeRealRows(rawRows, hist, cfg, overridesMap) {
  ...
  const resultRows = rawRows.map((row) => {
    ...
    try {
      ...
      const healthRes = clientCalc.calculateHealthScore(merged, prices);
      const ratingRes = clientCalc.calculateRating(merged, healthRes, cfg);
      // Current capital = sum over coins of (last known balance x price).
      let currentCapital = 0;
      const coinMap = (merged && merged.coins) || {};
      for (const coin of Object.keys(coinMap)) {
        const recents = coinMap[coin].recent_balances || [];
        const lastBal = recents.length ? (Number(recents[recents.length - 1]) || 0) : 0;
        const price = Number(prices[coin]) || 0;
        currentCapital += lastBal * price;
      }
      return Object.assign({}, row, {
        health_score: healthRes.health_score,
        rating: ratingRes.final_rating,
        rating_grade: ratingRes.letter_grade,
        // Coins actually used in the current calculation (non-zero in last 7d).
        calc_coins: Object.keys(healthRes.coins),
        usd_median: healthRes.total_usd_median,
...
async function openRealAdmin() {
  ...
  // All faucet URLs that have raw snapshots (authoritative for the raw tab),
  // merged with the faucets-table URLs so the dropdown is never incomplete.
  let realRawUrls = [];
  ...
  const overlay = openModalShell('⚙️ Управление кранами (Real Mode)', {
...
function buildRawTableHtml(snaps, healthByAt) {
  const coinOrder = [];
  const coinSet = new Set();
  ...
  let head = '<thead><tr><th>Время (UTC)</th><th>paid_today (USD)</th><th>total_users_paid</th><th>Current health</th>';
...
function renderRealRawData(container, faucetRows, rawUrls) {
  ...
  const note = document.createElement('div');
  note.className = 'lock-note';
  note.innerHTML = '🔒 Просмотр сырых почасовых снимков API FaucetPay. Столбцы: <code>Время (UTC)</code>, <code>paid_today (USD)</code> (сырое значение из ответа API, как есть), <code>total_users_paid</code> (сумма по монетам), <code>Current health</code> (новый независимый Health Score, рассчитанный ТОЛЬКО по <code>raw_hourly</code>: медиана пиков балансов за 7 дней как базис, среднее 3 последних снимков / базис — по весу в USD), далее для КАЖДОЙ монеты — столбец баланса и столбец <code>Курс &lt;монета&gt;</code> (в USD, на момент снимка, выделен зелёным), включая нулевые балансы. Таблица широкая — используйте горизонтальную прокрутку. Данные из Turso <code>raw_hourly</code> (последние 8 дней).';
  wrap.appendChild(note);
  ...
  async function loadAndRender() {
    ...
    // Compute the new parallel Health Score per snapshot from prior raw_hourly history.
    let healthByAt = {};
    try {
      const windowSnaps = await loadRawWindow(url, day, 7);
      const DAY7 = 7 * 86400000;
      snaps.forEach((s) => {
        const prev = windowSnaps.filter((p) => p.snapshot_at < s.snapshot_at && (new Date(s.snapshot_at) - new Date(p.snapshot_at)) <= DAY7);
        healthByAt[s.snapshot_at] = computeRawSnapshotHealth(prev, s);
      });
    } catch (e) {
      console.warn('Current health compute failed:', e);
    }

    tableHost.innerHTML = buildRawTableHtml(snaps, healthByAt);
  }
