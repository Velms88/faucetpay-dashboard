...
async function main() {
  ...
  // init_db never clobbers a config edited via the admin UI). The source file
  // faucet_config.json is optional now.
  ...
  if (!fs.existsSync(historyPath)) {
    ...
    for (const f of (hj.faucets || [])) {
      const meta = {
        ...
        gateways_count: f.gateways_count, uii: f.uii, bonus_points: f.bonus_points,
  ...
  console.log('\n==================== MIGRATION REPORT ====================');
  console.log('configs        :', await count('configs'), 'rows (prod)');
