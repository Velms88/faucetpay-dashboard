...
function enrichFaucets(raw, history, config, overridesMap, dailyVolByUrl) {
  ...
  try {
    ...
    for (const r of (rawRows || [])) {
      ...
      if (!cur || new Date(r.snapshot_at) >= new Date(cur.snapshot_at)) {
        latestHealthByUrl.set(u, { snapshot_at: r.snapshot_at, health_score: r.health_score });
      }
    }
    const pl = raw.data && typeof raw.data === 'object' ? raw.data : raw;
    const listData = pl.list_data;
    if (listData && typeof listData === 'object') {
      walkRecords(listData, (rec) => {
        const u = normalizeUrl(rec.url);
        if (latestHealthByUrl.has(u)) rec.health_score = latestHealthByUrl.get(u).health_score;
      });
    }
...
async function main() {
  ...
  // Блок 1 daily_volume: эталон из raw_hourly (если доступны креды Turso).
  ...
  try {
    ...
    console.warn('[turso] daily volume pre-load failed: ' + (e && e.message ? e.message : e));
  }

  const enrichResult = enrichFaucets(raw, history, config, overridesMap, dailyVolByUrl);
