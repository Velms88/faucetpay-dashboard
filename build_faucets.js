...
// Fetch prior raw_hourly snapshots (last 7 days) for a faucet and compute its
// Health Score for the given (new) snapshot row.
async function computeRawHealthForRow(client, rawRow) {
  const cutoff = new Date(Date.now() - 7 * 86400000).toISOString();
  let prior;
  try {
    prior = await client.execute({
      sql: 'SELECT snapshot_at, coins_json, prices_json FROM raw_hourly WHERE url = ? AND snapshot_at < ? AND snapshot_at >= ? ORDER BY snapshot_at ASC',
      args: [rawRow.url, rawRow.snapshot_at, cutoff],
    });
  } catch (e) {
    return null;
  }
  const prevSnaps = ((prior && prior.rows) || []).map((r) => ({
    snapshot_at: r.snapshot_at,
    coins: JSON.parse(r.coins_json || '[]'),
    prices: JSON.parse(r.prices_json || '{}'),
  }));
  const curSnap = {
    snapshot_at: rawRow.snapshot_at,
    coins: JSON.parse(rawRow.coins_json || '[]'),
    prices: JSON.parse(rawRow.prices_json || '{}'),
  };
  return computeRawSnapshotHealth(prevSnaps, curSnap);
}

async function writeRawToTurso(client, rows) {
  if (!rows || !rows.length) return 0;
  // Compute the new parallel Health Score for each snapshot from prior raw_hourly
  // history, attaching it to the row so it persists "the same way as other data".
  for (const r of rows) {
    try { r.health_score = await computeRawHealthForRow(client, r); }
    catch (e) { r.health_score = null; }
  }
...
function gatherInputCurrencySymbols(inputPath) {
  ...
  try {
    ...
    const listData = (raw.data && raw.data.list_data) || raw.list_data;
    walkRecords(listData, (rec) => {
...
async function main() {
  ...
  if (argv['fetch-prices'] || argv['fetchprices'] || argv['prices-only']) {
    ...
    const priceInputPath = (argv['prices-only'] && mode === MODE_SANDBOX)
      ? null
      : (argv.input || (config.settings && config.settings.data_file) || 'faucets.json');
