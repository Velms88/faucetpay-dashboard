  ...
  try {
    ...
    if (listData && typeof listData === 'object') {
      walkRecords(listData, (rec) => {
        ...
        // Берём ТОЛЬКО формульный health_score из raw_hourly (БД). Сырой показатель
        // FaucetPay из faucets.json категорически не используем: при отсутствии свежего
        // снимка ставим null, чтобы браузер пересчитал по формуле, а не показал чужое число.
