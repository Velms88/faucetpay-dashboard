...
async function writeRawToTurso(client, rows) {
  ...
  const incomingAts = new Set(rows.map((r) => r.snapshot_at));
  ...
  for (const r of newRows) {
    ...
    if (t > runningMax) runningMax = t;
      ...
      r.health_score = ref > 0 ? Math.round(Math.min(100, (t / ref) * 100)) : 0;
  }
...
async function mirrorToTurso(mode, raw, history, config, rawRows) {
  ...
  try {
    ...
    if (mode !== MODE_SANDBOX) {
      ...
      // Real Mode table now equals the latest raw_hourly CURRENT_HEALTH).
